...
// 弱不存在缓存文件则写入缓存
if(!file_exists(XAKDATA.'/enums/system.php')) WriteEnumsCache();
...
function WriteEnumsCache($egroup='')
{
    ...
    {
        $cachefile = XAKDATA.'/enums/'.$egroup.'.php';
...
{
    global ${'em_'.$egroup.'s'};
    include_once(XAKDATA.'/enums/'.$egroup.'.php');
    $jsCode = "<!--\r\n";
...
{
    $jsfile = XAKDATA.'/enums/'.$egroup.'.js';
    $fp = fopen($jsfile, 'w');
