...
@set_time_limit(0);
require(XAKINC.'/inc/inc_fun_funAdmin.php');
require(XAKINC.'/xakhttpdown.class.php');
...
//当前软件版本锁定文件
$verLockFile = XAKDATA.'/admin/ver.txt';
...
else if($dopost=='getlist')
{
    ...
    else
    {
        ...
        $allFileList .= "<div class='upinfotitle'>以下是需要下载的更新文件（路径相对于系统的根目录）：</div>\r\n";
        $filelists = explode("\n",$filelist);
        ...
        $allFileList .= "文件临时存放目录：../config/<input type='text' name='tmpdir' style='width:200px' value='$tmpdir' /><br />\r\n";
    ...
    include XAKINClude('templets/update_guide_getlist.htm');
    exit();
}
/**
下载文件（保存需下载内容列表）
...
{
    $cacheFiles = XAKDATA.'/cache/updatetmp.inc';
    ...
    else
    {
        ...
        {
            $tfilename = $filename;
            if( preg_match("#^xakadmin\/#i", $filename) ) 
            {
                $tfilename = preg_replace("#^xakadmin\/#", $adminDir.'/', $filename);
            }
    ...
    include XAKINClude('templets/update_guide_getfiles.htm');
    exit();
}
/**
下载文件，具体操作步骤
...
else if($dopost=='down')
{
    ...
    if(empty($startup))
    {
        ...
        $dhd->SaveToBin(XAKDATA.'/'.$tmpdir.'/'.$files[$curfile]);
        $dhd->Close();
    ...
    else
    {
        ...
        $dhd->Close();
        $truefile = XAKDATA.'/'.$tmpdir.'/sql.txt';
        $fp = fopen($truefile, 'w');
        fwrite($fp, $ct);
        fclose($fp);

        ShowMsg("完成所有远程文件获取操作：<a href='update_guide.php?dopost=apply'>&lt;&lt;点击此开始直接升级&gt;&gt;</a><br />你也可以直接使用[../config/{$tmpdir}]目录的文件手动升级。","javascript:;");
...
else if($dopost=='apply')
{
    ...
    else
    {
        ...
        if(isset($files) && is_array($files))
        {
            foreach($files as $f)
            {
                if(preg_match('#^xakadmin#', $f)) 
                {
                    $tf = preg_replace('#^xakadmin#', $adminDir, $f);
                }
                else {
                    $tf = $f;
                }
        ...
        if($badcp > 0)
        {
            $badmsg = "，其中失败 {$badcp} 个文件，<br />请从临时目录[../config/{$tmpdir}]中取出这几个文件手动升级。";
        }
