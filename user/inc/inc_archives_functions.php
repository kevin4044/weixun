 ...
 */
if(!defined('XAKMEMBER')) exit('Warning...');
...
function GetCurContentAlbum($body,$rfurl,&$firstdd)
{
    ...
    include_once(XAKINC."/xakcollection.func.php");
    ...
    foreach($img_array as $key => $value)
    {
        ...
        {
            $rsimg .= "{xak:img ddimg='$value' text='' width='0' height='0'} $value {/xak:img}\r\n";
        }
    }
    return $rsimg;
}
...
function PrintAutoFieldsAdd(&$fieldset, $loadtype='all', $isprint=TRUE)
{
    ...
    if ($isprint) echo "<input type='hidden' name='xak_addonfields' value=\"".$xak_addonfields."\">\r\n";
    echo "<input type=\"hidden\" name=\"xak_fieldshash\" value=\"".md5($xak_addonfields.$cfg_cookie_encode)."\" />";
    // 增加一个返回
...
function MakeArt($aid, $ismakesign=FALSE)
{
    ...
    include_once(XAKINC.'/arc.archives.class.php');
    if($ismakesign)
    {
        $envs['makesign'] = 'yes';
    }
