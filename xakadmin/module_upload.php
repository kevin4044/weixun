...
if($action=='upload')
{
    ...
    {
        include_once(dirname(__FILE__)."/../functions/zip.class.php");
        ...
        if($filetype==1)
        {
            ...
            //为了节省资源，系统仅以.xml作为扩展名识别ZIP包里的模块格式文件
            if(is_array($files))
            {
            ...
            {
                unlink($tmpfilename);
                ShowMsg("对不起，你上传的压缩包中不存在模块文件！<br /><br /><a href='javascript:history.go(-1);'>&gt;&gt;返回重新上传&gt;&gt;</a>","javascript:;");
                exit();
            }
            $ziptmp = $mdir.'/ziptmp';
            $z->Extract($tmpfilename, $ziptmp, $xakfileindex);
            unlink($tmpfilename);
            $tmpfilename = $mdir."/ziptmp/".$xakfile;
        }
        ...
        if(empty($infos['hash']))
        {
            unlink($tmpfilename);
            $dm->Clear();
            ShowMsg("对不起，你上传的文件可能不是模块的标准格式文件！<br /><br /><a href='javascript:history.go(-1);'>&gt;&gt;返回重新上传&gt;&gt;</a>","javascript:;");
            exit();
        }
