...
if(!$ways=='' && !$body=='')
{
    ...
    break;
    case "userid": 
        $row=$dsql->GetOne("SELECT mid FROM `#@__member_list` WHERE userid='$body' LIMIT 1");
        $mid=$row['mid'];
        $where .="AND g.mid='$mid'";
    break;
    case "msg": 
...
$sql = "SELECT g.*,m.userid FROM `#@__member_guestbook` AS g LEFT JOIN `#@__member_list` AS m ON g.mid=m.mid WHERE 1=1 $where ORDER BY aid DESC";
$dlist = new DataListCP();
$dlist->pageSize = 20;
$dlist->SetTemplate(XAKADMIN."/templets/member_guestbook.htm");
$dlist->SetSource($sql);
$dlist->Display();