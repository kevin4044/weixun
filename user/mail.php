...
if($dopost == '')
{
	if($cfg_mb_spacesta!="-10"){
		if($cfg_checkemail=='Y'){
			$row=$dsql->GetOne("SELECT email,checkmail FROM `#@__member_list` WHERE mid=$mid");
			if($row['checkmail']=="-1"){
	...
	if($folder=="drop"){
	  ...
	  $tpl = new XakTemplate();
	  $tpl->LoadTemplate(XAKMEMBER.'/templets/mail_drop.htm');
	  $tpl->Display();
	}else{
	  ...
	  $tpl = new XakTemplate();
	  $tpl->LoadTemplate(XAKMEMBER.'/templets/mail.htm');
	  $tpl->Display();
	} 
  ...
  $url = $cfg_basehost.(empty($cfg_cmspath) ? '/' : $cfg_cmspath)."/user/mail.php?dopost=checkmail&mid={$cfg_ml->fields['mid']}&userhash={$userhash}&do=1";
  ...
  $mailbody .= "{$url}\r\n\r\n";
	...
	$dsql->ExecuteNoneQuery("Update `#@__member_list` set checkmail=0 where mid='{$mid}' ");
