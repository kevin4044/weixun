...
if($tid==0 && $channelid==0) die(" Warning... ");
...
if($tinfos['issystem']==-1)
{
    ...
    include(XAKINC."/arc.sglistview.class.php");
    $lv = new SgListView($tid,$cArr);
} else {
    include(XAKINC."/arc.listview.class.php");
    $lv = new ListView($tid);
    //对设置了会员级别的栏目进行处理
    if(isset($lv->Fields['corank']) && $lv->Fields['corank'] > 0)
    {
        ...
        if( $cfg_ml->M_Rank < $lv->Fields['corank'] )
        {
            ...
            include_once(XAKTEMPLATE.'/module/view_msg_catalog.htm');
            exit();
        }
    }
}
