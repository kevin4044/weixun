...
//生成必须同步服务器的列表
if(file_exists(XAKDATA.'/config.file.inc.php'))
{
    require_once(XAKDATA.'/config.file.inc.php');
}
...
{
    //将其转换为配置写入文件
    $configfile = XAKDATA.'/config.file.inc.php';
...
else if($dopost == 'updateremote')
{
    ...
    //config/js目录
    $dirarray[$i++] = addDir('/config/js', '生成js目录', 0, 0, 1);
    //$remotefile = array();
    //把非系统目录的内容提取出来
...
else if($dopost == 'make')
{
    if($step == 1)
    {
        ...
        $cacheMakeFile = XAKDATA.'/cache/filelist.inc.php';
        ...
        if(count($Iterm) > 0)
        {
            ...
            foreach($Iterm as $key => $val)
            {
                ...
                if(is_array($dirlist = getDirs(XAKROOT.$val)))
                {
    ...
    {
        if(file_exists(XAKDATA.'/cache/filelist.inc.php'))
        {
            require_once(XAKDATA.'/cache/filelist.inc.php');
        }
        if(is_array($dirlist))
        {
            if($sta > 0 && $sta < $totalnum)
            {
                ...
                if($ftp->connect($config))
                {
                    ...
                    {
                        //如果是文件目录
                        $remotedir = str_replace(XAKROOT, '', $dirname).'/';
                        $localdir = '..'.$remotedir.'/';
                        ...
                        $remotefile = str_replace(XAKROOT, '', $dirname);
                        ...
                        {
                            list($filename,$fileconfig) = explode('|', $val); 
                            if(preg_replace('/[^\/]*\.(\w){0,}/', '', str_replace(XAKROOT, '', $filename)) == $remotedir)
                            {
                ...
                @unlink(XAKDATA.'/cache/filelist.inc.php');
                echo '全部同步完毕!';exit;
            }
        } else {
...
include XAKINClude('templets/makeremote_all.htm');