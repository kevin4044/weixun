 ...
 */
define('XAKADMIN', str_replace("\\", '/', dirname(__FILE__) ) );
require_once(XAKADMIN.'/../functions/common.inc.php');
require_once(XAKADMIN.'/conn.php');
...
if(file_exists(XAKDATA.'/admin/skin.txt'))
{
	$skin = file_get_contents(XAKDATA.'/admin/skin.txt');
	$cfg_admin_skin = !in_array($skin, array(1,2,3,4))? 1 : $skin;
}

//获得当前脚本名称，如果你的系统被禁用了$_SERVER变量，请自行更改这个选项
$xakNowurl = $s_scriptName = '';
$isUrlOpen = @ini_get('allow_url_fopen');
$xakNowurl = GetCurUrl();
$xakNowurls = explode('?', $xakNowurl);
$s_scriptName = $xakNowurls[0];
...
{
    header("location:login.php?gotopage=".urlencode($xakNowurl));
    exit();
}
...
if($cfg_xak_log=='Y')
{
    ...
    $s_query = isset($xakNowurls[1]) ? $xakNowurls[1] : '';
...
if($cfg_remote_site=='Y')
{
    ...
    if(file_exists(XAKDATA."/cache/inc_remote_config.php"))
    {
        require_once XAKDATA."/cache/inc_remote_config.php";
    }
...
//管理缓存、管理员频道缓存
$cache1 = XAKDATA.'/cache/inc_catalog_base.inc';
if(!file_exists($cache1)) UpDateCatCache();
$cacheFile = XAKDATA.'/cache/admincat_'.$cuserLogin->userID.'.inc';
if(file_exists($cacheFile)) require_once($cacheFile);

//更新服务器
require_once (XAKDATA.'/admin/config_update.php');
...
function UpDateCatCache()
{
    ...
    $cache2 = XAKDATA.'/cache/channelsonlist.inc';
    $cache3 = XAKDATA.'/cache/channeltoplist.inc';
...
{
    global $dsql;
    $cachefile = XAKDATA.'/cache/member_model.inc';
 ...
 * @return    string
 */
function XAKINClude($filename, $isabs=FALSE)
{
    return $isabs ? $filename : XAKADMIN.'/'.$filename;
}
...
if(!function_exists('GetMemberName')){
	function GetMemberName($mid=0)
	{
		...
		if( empty($rs) )
		{
			$rs = $dsql->GetOne("SELECT * FROM `#@__member_list` WHERE mid='{$mid}' ");
