 ...
 */
require_once(XAKINC.'/arc.partview.class.php');
...
class ListView
{
    ...
    function __construct($typeid, $uppage=1)
    {
        ...
        if(!$this->IsError)
        {
            ...
            $this->Fields['rsslink'] = $GLOBALS['cfg_cmsurl']."/config/rss/".$this->TypeID.".xml";
    ...
    function MakeHtml($startpage=1, $makepagesize=0, $isremote=0)
    {
        ...
        {
            echo '这个类目是动态类目！';
            return '../module/list.php?tid='.$this->TypeLink->TypeInfos['id'];
        }
		//链接到第一个子分类
        if($this->TypeLink->TypeInfos['isdefault']==2)
        {
            echo '这个类目链接到第一个子分类！';
            return '../module/list.php?tid='.$this->TypeLink->TypeInfos['id'];
        }
        ...
        for($this->PageNo=$startpage; $this->PageNo < $endpage; $this->PageNo++)
        {
            ...
            {
                //分析远程文件路径
                $remotefile = str_replace(XAKROOT, '',$makeFile);
    ...
    $imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
    {
        ...
        if($orderby=="senddate" || $orderby=="id") {
    		...
    		$ordersql = " order by arc.weight $orderWay ,arc.id $orderWay";  
		}
        else {
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno",$site_lang)
    {
		...
		if($site_lang == 'cn' || $site_lang =='')
		{
			...
			{
				$indexpage="<li><a href='javascript:void(0)'>首页</a></li>\r\n";
			}
			...
			{
				$endpage="<li><a href='javascript:void(0)'>末页</a></li>\r\n";
			}
		}
		elseif ($site_lang == 'en')
		{
			if($totalpage<=1 && $this->TotalResult>0)
			{
				...
				return "<li><span class=\"pageinfo\">Total <strong>1</strong> Pages <strong>".$this->TotalResult."</strong> Records</span></li>\r\n";
			}
			if($this->TotalResult == 0)
			{
				return "<li><span class=\"pageinfo\">Total <strong>0</strong> Pages<strong>".$this->TotalResult."</strong> Records</span></li>\r\n";
			}
			$purl = $this->GetCurUrl();
			$maininfo = "<li><span class=\"pageinfo\">Total <strong>{$totalpage}</strong> Pages<strong>".$this->TotalResult."</strong> Records</span></li>\r\n";
			$tnamerule = $this->GetMakeFileRule($this->Fields['id'],"list",$this->Fields['typedir'],$this->Fields['defaultname'],$this->Fields['namerule2']);
			$tnamerule = preg_replace("/^(.*)\//", '', $tnamerule);
			...
			{
				$indexpage="<li><a href='javascript:void(0)'>Home</a></li>\r\n";
			}
			...
			{
				$endpage="<li><a href='javascript:void(0)'>End</a></li>\r\n";
			}
		}
        ...
        for($j;$j<=$total_list;$j++)
        {
            if($j==$this->PageNo)
            {
                $listdd.= "<li class=\"thisclass\"><a href='javascript:void(0)'>$j</a></li>\r\n";
            }
            else
            {
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno",$site_lang)
    {
		...
		elseif($site_lang == 'en')
		{
			...
			$maininfo = "<li><span class=\"pageinfo\">TOTALS <strong>{$totalpage}</strong>Pages<strong>".$this->TotalResult."</strong>Records</span></li>\r\n";
