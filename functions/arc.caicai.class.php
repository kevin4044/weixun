...
class Caicai extends DataListCP
{
    ...
    function GetArcList($atts, $refObj='', $fields=array())
    {
        ...
        while($arr=$this->dsql->GetArray('dlist'))
        {
            ...
            if($arr['userid']!='')
            {
                $arr['spaceurl'] = $GLOBALS['cfg_basehost'].'/user/index.php?uid='.$arr['userid'];
            }
            else
            {
            ...
            {
                $arr['lastpost'] = "<a href='../module/feedback.php?aid={$arr['id']}'>说几句&gt;&gt;</a>";
            }
            $rsArray[$i]  =  $arr;
            if($i >= $this->pageSize)
            {
    ...
    function GetSortArc($atts, $refObj='', $fields=array())
    {
        ...
        $cacheFile = XAKDATA.'/cache/caicai_'.md5($query).'.inc';
        $needCache = false;
