...
require_once(XAKINC."/charset.func.php");
...
function TurnImageTag(&$body)
{
    ...
    foreach ( $returnArray1 as $key => $value )
    {
        if ($value)
        {
          $ttx .= "{xak:img ddimg='$litpicname' text='图 ".($key+1)."'}".$value."{/xak:img}"."\r\n";
        }
        else
        {
          $ttx .= "{xak:img ddimg='$litpicname' text='图 ".($key+1)."'}".$returnArray2[$key]."{/xak:img}"."\r\n";
        }
    }
    $ttx = "\r\n{xak:pagestyle maxwidth='{$cfg_album_width}' ddmaxwidth='{$cfg_ddimg_width}' row='3' col='3' value='2'/}\r\n{xak:comments}图集类型会采集时生成此配置是正常的，不过如果后面没有跟着img标记则表示规则无效{/xak:comments}\r\n".$ttx;
    return $ttx;
}
...
function TurnLinkTag(&$body)
{
    ...
    if(is_array($match[1]) && count($match[1])>0)
    {
        for($i=0;isset($match[1][$i]);$i++)
        {
            ...
            $ttx .= "{xak:link text='$servername'} {$match[1][$i]} {/xak:link}\r\n";
        }
    }
    return $ttx;
}
