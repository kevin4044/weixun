...
require_once(XAKADMIN.'/inc/inc_list_functions.php');

//检查权限许可，总权限
CheckPurview('a_List,a_AccList,a_MyList');

if(!isset($upate_weight)) $upate_weight = '';
...
{
	//设定跳转地址
	$mid = isset($pageno) ? intval($pageno) : 0;
	$mid = isset($totalresult) ? intval($totalresult) : 0;
	$page_url = '';
	if($totalresult > 0)
	{
		$page_url .= '&totalresult='.$totalresult.'&pageno='.$pageno;
	}	
	if($s_tmplets !='')
	{
		$page_url .= '&totalresult='.$totalresult.'&pageno='.$pageno.'&s_tmplets='.$s_tmplets;
	}
	$local_url = 'content_list.php?dopost=listArchives&keyword='.$keyword.'&cid='.$cid.'&flag='.$flag.'&orderby='.$orderby.'&arcrank='.$arcrank.'&channelid='.$channelid.$page_url;
	...
	if(is_array($weight))
    {
		...
		{
			$dsql->ExecuteNoneQuery("UPDATE #@__archives SET weight='".$val."' WHERE id='{$key}';");
		}
    }
    ShowMsg("操作成功，正在返回...",$local_url);
    exit();
}
else
{
  ...
  $optCache = XAKDATA."/tplcache/inc_option_$optHash.inc";
  ...
  $query = "SELECT arc.id,arc.weight,arc.typeid,arc.senddate,arc.flag,arc.ismake,
  ...
  $dlist->SetTemplate(XAKADMIN.'/'.$s_tmplets);
