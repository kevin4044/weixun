...
if($uid=='')
{
    ...
    else
    {
        ...
        LEFT JOIN `#@__member_list` mem ON mem.mid=arc.mid
        WHERE arc.arcrank > -1
        ...
        /** 欢迎新朋友 **/
        $sql = "SELECT * FROM `#@__member_list` ORDER BY mid DESC LIMIT 3";
        $newfriends = array();
        $dsql->SetQuery($sql);
        $dsql->Execute();
        while ($row = $dsql->GetArray()) {
        ...
        /** 好友记录 **/
        $sql = "SELECT F.*,M.face,M.sex FROM `#@__member_list` AS M LEFT JOIN `#@__member_friends` AS F ON F.fid=M.mid WHERE F.mid='{$cfg_ml->M_ID}' ORDER BY F.addtime desc LIMIT 6";
        $friends = array();
        $dsql->SetQuery($sql);
        $dsql->Execute();
        while ($row = $dsql->GetArray()) {
...
{
    require_once(XAKMEMBER.'/inc/config_space.php');
    if($action == '')
    {
        include_once(XAKINC."/channelunit.func.php");
        $dpl = new XakTemplate();
        $tplfile = XAKMEMBER."/space/{$_vars['spacestyle']}/index.htm";
    ...
    {
        require_once(XAKMEMBER.'/inc/space_action.php');
        exit();
    }
}