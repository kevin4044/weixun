...
function lib_channel(&$ctag,&$refObj)
{
    ...
    if($type=='top')
    {
        $sql = "SELECT id,typename,typedir,seotitle,content,typepic,open_new,description,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath
	...
	{
		if($type=='son' && $reid!=$ntop && $totalRow==0 && $reid !=0)
		{
			$sql = "SELECT id,typename,typedir,seotitle,content,typepic,open_new,description,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath
				FROM `#@__arctype` WHERE reid='$reid' And ishidden<>1 order by sortrank asc limit 0, $line ";
			$dsql->SetQuery($sql);
