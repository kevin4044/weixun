...
else if($dopost=="export")
{
    ...
    $row = $dsql->GetOne("SELECT * FROM `#@__channeltype` WHERE id='$id' ");
    $channelconfig = '';
    $row['maintable'] = preg_replace('#xak_#', '#@__', $row['maintable']);
    $row['addtable'] = preg_replace('#xak_#', '#@__', $row['addtable']);
    foreach($row as $k=>$v)
    {
        if($k=='fieldset') $v = "\r\n$v\r\n";
...
{
    require_once(XAKADMIN."/inc/inc_admin_channel.php");
...
else if($dopost == 'modifysearch')
{
    ...
    } else if ($step == 1)
    {
        ...
        $forms = '<form action="'.$cfg_cmspath.'/module/advancedsearch.php" method="post">';
        ...
        if(!empty($mainfields) && is_array($mainfields))
        {
            ...
            foreach($mainfields as $mainfield)
            {
                if($mainfield == 'typeid')
                {
                    require_once(dirname(__FILE__)."/../functions/typelink.class.php");
                    $tl = new TypeLink(0);
...
$row = $dsql->GetOne("SELECT * FROM `#@__channeltype` WHERE id='$id' ");
require_once(XAKADMIN."/templets/mychannel_edit.htm");