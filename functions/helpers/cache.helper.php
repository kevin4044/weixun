...
if ( ! function_exists('GetCache'))
{
    function GetCache($prefix, $key, $is_memcache = TRUE)
    {
        ...
        $result = @file_get_contents ( XAKDATA . "/cache/$prefix/$key.php" );
        
        if ($result === false)
        {
            return false;
        }
        $result = str_replace("<?php exit('Warning...');?>\n\r", "", $result);
...
if ( ! function_exists('SetCache'))
{
    function SetCache($prefix, $key, $value, $timeout = 3600, $is_memcache = TRUE)
    {
        ...
        $cache_data = "<?php exit('Warning...');?>\n\r".@serialize ( $tmp );
        return @PutFile ( XAKDATA . "/cache/$prefix/$key.php",  $cache_data);
    }
}
...
if ( ! function_exists('DelCache'))
{
    ...
    function DelCache($prefix, $key, $is_memcache = TRUE)
    {
        ...
        $key = substr ( $key, 0, 2 ) . '/' . substr ( $key, 2, 2 ) . '/' . substr ( $key, 4, 2 ) . '/' . $key;
        return @unlink ( XAKDATA . "/cache/$prefix/$key.php" );
    }
}