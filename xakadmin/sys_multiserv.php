...
if($dopost=="save")
{
    $configfile = XAKDATA."/cache/inc_remote_config.php";
...
//获取会员列表
$query = "SELECT `#@__admin_list`.*,`#@__admintype`.typename FROM `#@__admin_list` LEFT JOIN `#@__admintype` ON `#@__admin_list`.usertype = `#@__admintype`.rank";
$dsql->SetQuery($query);
$dsql->Execute();
...
include XAKINClude('templets/sys_multiserv.htm');
exit();