...
$attachFullDir = str_replace("\\", '/', dirname(__FILE__) ).'/../uploads/allimg';//上传文件保存路径，结尾不要带/
$attachDir = '/uploads/allimg/';//上传文件保存路径，结尾不要带/
...
$err = "";
$msg = "''";
$tempPath=$attachFullDir.'/'.date("YmdHis").mt_rand(10000,99999).'.tmp';
$localName='';
...
if($err==''){
	...
	if(preg_match('/'.str_replace(',','|',$upExt).'/i',$extension))
	{
		...
		else
		{
			...
			$attachFullDir = $attachFullDir.'/'.$attachSubDir;
			if(!is_dir($attachFullDir))
			{
				@mkdir($attachFullDir, 0777);
				@fclose(fopen($attachFullDir.'/index.htm', 'w'));
			}
			PHP_VERSION < '4.2.0' && mt_srand((double)microtime() * 1000000);
			$newFilename=date("YmdHis").mt_rand(1000,9999).'.'.$extension;
			$targetPath = $attachFullDir.'/'.$newFilename;
			
			rename($tempPath,$targetPath);
			@chmod($targetPath,0755);
			$targetPath=jsonString($targetPath);
			if($immediate=='1')$targetPath='!'.$targetPath;
			$targetPath = $attachDir.$attachSubDir.'/'.$newFilename;
