...
//当前软件指纹码锁定文件
$verifiesLockFile = XAKDATA.'/admin/verifies.txt';
...
if($action == '')
{
    include(XAKADMIN.'/templets/sys_verifies.htm');
    exit();
}
/*----------------
校验文件
...
else if($action == 'verify')
{
    ...
    {
        $turefile = str_replace('../xakadmin', '.', $row['filename']);
        //跳过不存在的文件
    ...
    {
        include(XAKADMIN.'/templets/sys_verifies_verify.htm');
    }
    exit();
}
/*--------------------
...
else if ($action == 'manage')
{
    ...
    include(XAKADMIN.'/templets/sys_verifies_manage.htm');
    exit();
}
/*-----------------------
下载文件
...
else if ($action == 'getfiles')
{
    ...
    $cacheFiles = XAKDATA.'/modifytmp.inc';
    ...
    foreach($refiles as $filename)
    {
        ...
        {
            $curdir = GetDirName( preg_replace("#^xakadmin/#i", $adminDir.'/', $filename) );
        } else {
    ...
    include(XAKADMIN.'/templets/sys_verifies_getfiles.htm');
...
else if($action=='modify')
{
    ...
    else
    {
        foreach($modifys as $fname)
        {
            if($method=='local')
            {
                $tureFilename = str_replace('../xakadmin','./',$fname);
...
else if ($action == 'apply')
{
    ...
    if(isset($files) && is_array($files))
    {
        foreach($files as $f)
        {
            if(preg_match("#^xakadmin#", $f)) $tf = preg_replace("#^xakadmin#", $adminDir, $f);
            else $tf = $f;
    ...
    if($badcp > 0)
    {
        $badmsg = "，其 {$badcp} 个文件效验码不正确或复制失败，<br />请从临时目录[../config/{$tmpdir}]中取出这几个文件手动还原。";
    }
