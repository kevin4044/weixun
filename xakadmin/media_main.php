...
FROM `#@__uploads` u
LEFT JOIN `#@__admin_list` a ON  a.id = u.mid
LEFT JOIN `#@__member_list` m ON m.mid = u.mid
...
$dlist->SetTemplate(XAKADMIN."/templets/media_main.htm");
$dlist->SetSource($sql);
$dlist->Display();
...
function MediaType($tid,$nurl)
{
    if($tid==1)
    {
        return "图片<a href=\"javascript:;\" onClick=\"ChangeImage('$nurl');\"><img src='../functions/dialog/img/picviewnone.gif' name='picview' border='0' alt='预览'></a>";
    }
    else if($tid==2)
    {
        return "FLASH";
