 ...
 * 模板引擎文件
 *
 * @version        $Id: xaktemplate.class.php 3 15:44 2010年7月6日
 */
...
function MakePublicTag($atts=array(),$refObj='',$fields=array())
{
    ...
    $plusfile = XAKINC.'/tpllib/plus_'.$atts['tagname'].'.php';
...
class XakTemplate
{
    ...
    var $refObj = '';
    var $makeLoop = 0;
    var $tagStartWord =  '{xak:';
    var $fullTagEndWord =  '{/xak:';
    var $sTagEndWord = '/}';
    var $tagEndWord = '}';
    var $tpCfgs = array();
    ...
    function __construct($templatedir='',$refDir='')
    {
        ...
        if($templatedir=='')
        {
            $this->templateDir = XAKROOT.'/templets';
        }
        else
        {
        ...
        $this->cacheDir = XAKROOT.$GLOBALS['cfg_tplcache_dir'];
    }

    //构造函数,兼容PHP4
    function XakTemplate($templatedir='',$refDir='')
    {
     ...
     * @return    void
     */
    function SetTagStyle($ts='{xak:',$ftend='{/xak:',$stend='/}',$tend='}')
    {
...
// ------------------------------------------------------------------------
    ...
    function CompilerInclude($filename, $isload=TRUE)
    {
        ...
        if($okfile=='') return 0;
        if( !$isload ) return 1;
        $itpl = new XakTemplate($this->templateDir);
