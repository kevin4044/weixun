...
{
    $cacheFile = XAKDATA.'/cache/addon-'.session_id().'.inc';
...
class userLogin
{
    ...
    var $keepUserTypeTag = 'admin_type';
    var $keepUserChannelTag = 'admin_channel';
    var $keepUserNameTag = 'admin_name';
    var $keepUserPurviewTag = 'admin_purview';
    var $keepAdminStyleTag = 'admin_style';
    var $adminStyle = 'xak.cn';
    ...
    function checkUser($username, $userpwd)
    {
		...
		$pwd = PassJoin($this->userPwd);
        //$pwd = substr(md5($this->userPwd), 5, 20);
        $dsql->SetQuery("SELECT admin.*,atype.purviews FROM `#@__admin_list` admin LEFT JOIN `#@__admintype` atype ON atype.rank=admin.usertype WHERE admin.userid LIKE '".$this->userName."' LIMIT 0,1");
        $dsql->Execute();
        $row = $dsql->GetObject();
        if(!isset($row->pwd))
        {
        ...
        else
        {
            ...
            $inquery = "UPDATE `#@__admin_list` SET loginip='$loginip',logintime='".time()."' WHERE id='".$row->id."'";
            $dsql->ExecuteNoneQuery($inquery);
            $sql = "UPDATE `#@__member_list` SET logintime=".time().", loginip='$loginip' WHERE mid=".$row->id;
    ...
    function keepUser()
    {
        if($this->userID != '' && $this->userType != '')
        {
            ...
            if(empty($adminstyle)) $adminstyle = 'xak.cn';
            ...
            PutCookie('XakUserID', $this->userID, 3600 * 24, '/');
            PutCookie('XakLoginTime', time(), 3600 * 24, '/');
    ...
    {
        //$this->userChannel
        $cacheFile = XAKDATA.'/cache/admincat_'.$this->userID.'.inc';
        //管理员管理的频道列表
...
function GetSonIdsUL($id, $channel=0, $addthis=TRUE)
{
    ...
    if( !is_array($cfg_Cs) )
    {
        require_once(XAKDATA."/cache/inc_catalog_base.inc");
    }
