...
if($fmdo=='sendMail')
{
    ...
    $mailbody .= "{$url}\r\n\r\n";
    $mailbody .= "Power by http://www.xak.cn 内容管理系统！\r\n";
...
else if($fmdo=='checkMail')
{
    ...
    $row = $dsql->GetOne("SELECT * FROM `#@__member_list` WHERE mid='{$mid}' ");
    ...
    $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` SET spacesta=0 WHERE mid='{$mid}' ");
    // 清除会员缓存
    $cfg_ml->DelCache($mid);
...
else if($fmdo=='user')
{
    ...
    else  if($dopost=="checkmail")
    {
        ...
        else
        {
            ...
            {
                 $row = $dsql->GetOne("SELECT mid FROM `#@__member_list` WHERE email LIKE '$email' LIMIT 1");
                 if(!is_array($row)) {
                     $msg = "<font color='#4E7504'><b>√可以使用</b></font>";
                 }
                 else {
  ...
  ***************************/
    else if($dopost=="money2s")
    {
        ...
        //修改积分与金币值
        $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` SET `scores`=$litmitscores, money= money + $money  WHERE mid='".$cfg_ml->M_ID."' ");
        ...
        if(defined('UC_API') && @include_once XAKROOT.'/uc_client/client.php')
        {
            ...
            if($uid > 0) {
                ...
                if(!$rs) {
                    ...
                    $res = $dsql->ExecuteNoneQuery("INSERT INTO `#@__member_list` SET `mtype`='个人',`userid`='$username',`pwd`='$password',`uname`='$username',`sex`='男' ,`rank`='10',`money`='$money', `email`='$email', `scores`='$scores', `matt`='0', `face`='',`safequestion`='0',`safeanswer`='', `jointime`='$jointime',`joinip`='$joinip',`logintime`='$logintime',`loginip`='$loginip';");
                    if($res) {
                        $mid = $dsql->GetLastID();
                        $data = array
                        (
                       ...
                       `drink`='0', `datingtype`='0', `language`='', `nature`='', `lovemsg`='', `address`='',`uptime`='0';",
                        ...
                        3 => "INSERT INTO `#@__member_flink` SET `mid`='$mid', `title`='内容管理系统', `url`='http://www.xak.cn';"
                        );                        
                        foreach($data as $val) $dsql->ExecuteNoneQuery($val);
                    }
                }
                $rs = 1;
                $row = $dsql->GetOne("SELECT `mid`, `pwd` FROM `#@__member_list` WHERE `userid`='$username'");
                if(isset($row['mid']))
                {
                    $cfg_ml->PutLoginInfo($row['mid']);
                    if($password!=$row['pwd']) $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` SET `pwd`='$password' WHERE mid='$row[mid]'");
                }
                //生成同步登录的代码
                ...
                if($rs) {
                    $row = $dsql->GetOne("SELECT `email` FROM `#@__member_list` WHERE userid='$userid'");                    
