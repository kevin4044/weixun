...
function CheckUserID($uid, $msgtitle='用户名', $ckhas=TRUE)
{
    ...
    if($ckhas)
    {
        $row = $dsql->GetOne("SELECT * FROM `#@__member_list` WHERE userid LIKE '$uid' ");
...
class MemberLogin
{
    ...
    function __construct($kptime = -1, $cache=FALSE)
    {
        ...
        $formcache = FALSE;
        $this->M_ID = $this->GetNum(GetCookie("XakUserID"));
        $this->M_LoginTime = GetCookie("XakLoginTime");
        $this->fields = array();
        $this->isAdmin = FALSE;
        if(empty($this->M_ID))
        {
            ...
            if ($cache)
            {
                ...
                {
                    $this->fields = $dsql->GetOne("Select * From `#@__member_list` where mid='{$this->M_ID}' ");
                } else {
                    $formcache = TRUE;
                }
            } else {
            ...
            if(is_array($this->fields)){
                #api{{
                if(defined('UC_API') && @include_once XAKROOT.'/uc_client/client.php')
                {
                    if($data = uc_get_user($this->fields['userid']))
                    {
                        if(uc_check_avatar($data[0]) && !strstr($this->fields['face'],UC_API))
                        {
                            ...
                            $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` SET `face`='".$this->fields['face']."' WHERE `mid`='{$this->M_ID}'");
                        }
                    }
                }
                #/aip}}
                ...
                {
                    $dsql->ExecuteNoneQuery("update `#@__member_list` set logintime='".time()."',loginip='".GetIP()."' where mid='".$this->fields['mid']."';");
                    PutCookie("XakLoginTime",time(),$this->M_KeepTime);
                }
    ...
    function Judgemember()
    {
        ...
        if($mhasDay <= 0){
           $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` SET uptime='0',exptime='0',
    ...
    function ResetUser()
    {
        ...
        $this->M_JoinTime = 0;
        $this->M_HasDay = 0;
        DropCookie('XakUserID');
        DropCookie('XakLoginTime');
    }
    ...
    function CheckUser(&$loginuser, $loginpwd)
    {
        ...
        $row = $dsql->GetOne("SELECT mid,matt,pwd,logintime FROM `#@__member_list` WHERE userid LIKE '$loginuser' ");
        if(is_array($row))
        {
            if($row['pwd'] != PassJoin($loginpwd))
            {
                return -1;
            }
            else
    ...
    function PutLoginInfo($uid, $logintime=0)
    {
        ...
        {
            $dsql->ExecuteNoneQuery("Update `#@__member_list` set `scores`=`scores`+{$cfg_login_adds} where mid='$uid' ");
        }
        $this->M_ID = $uid;
        $this->M_LoginTime = time();
        $loginip = GetIP();
        $inquery = "UPDATE `#@__member_list` SET loginip='$loginip',logintime='".$this->M_LoginTime."' WHERE mid='".$uid."'";
        $dsql->ExecuteNoneQuery($inquery);
        if($this->M_KeepTime > 0)
        {
            PutCookie('XakUserID',$uid,$this->M_KeepTime);
            PutCookie('XakLoginTime',$this->M_LoginTime,$this->M_KeepTime);
        }
        else
        {
            PutCookie('XakUserID',$uid);
            PutCookie('XakLoginTime',$this->M_LoginTime);
        }
    }
    ...
    function GetSta($dsql)
    {
        ...
        if($this->M_Rank==0)
        {
            ...
            $rs = $dsql->GetOne("Select id From `#@__admin_list` where userid='".$this->M_LoginID."'");
            if(!is_array($rs)){
    ...
    function RecordFeeds($type, $title, $note, $aid)
    {
        ...
        if (in_array($type,array('add','addsoft','feedback','addfriends','stow'))){
            ...
            if(in_array($type,array('add','addsoft','feedback','stow')))
            {
                ...
                //内容发布处理
                $arcrul = " <a href='/module/view.php?aid=".$aid."'>".$title."</a>";
                $title = htmlspecialchars($rcdtype[$type].$arcrul, ENT_QUOTES);
            } else if ($type == 'addfriends')
            {
                //添加好友处理
                $arcrul = " <a href='/user/index.php?uid=".$aid."'>".$aid."</a>";
