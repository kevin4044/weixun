 ...
 * 模块类
 *
 * @version        $Id: xakmodule.class.php 1 10:31 2010年7月6日
 
 */
require_once(XAKINC.'/charset.func.php');
require_once(XAKINC.'/xakatt.class.php');
...
class XakModule
{
    ...
    function XakModule($modulespath='')
    {
    ...
    function GetModuleUrlList($moduletype='',$url='')
    {
		$dh = dir($this->modulesPath) or die("没找到模块目录：({$this->modulesPath})！");
        $fp = @fopen($this->modulesPath.'/modulescache.php','w') or die('读取文件权限出错,目录文件'.$this->modulesPath.'/modulescache.php不可写!');
		$cachefile = XAKDATA.'/module/moduleurllist.txt';
		$remotelist = '';
