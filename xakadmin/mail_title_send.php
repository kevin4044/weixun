...
if($dopost=="send"){
	...
	$sql="SELECT m.email FROM `#@__member_list` AS m LEFT JOIN `#@__mail_order` AS o ON o.mid=m.mid WHERE o.typeid=$typeid";
	$db->Execute('me',$sql);
  ...
  if($dsql->ExecuteNoneQuery($inquery)){
	...
	$dlist->SetTemplet(XAKADMIN."/templets/mail_title_send.htm");
	$dlist->SetSource($sql);
	$dlist->display();
}
