...
require_once(XAKINC."/customfields.func.php");
require_once(XAKADMIN."/inc/inc_archives_functions.php");

if(empty($dopost)) $dopost = '';

if($dopost!='save')
{
    require_once(XAKADMIN."/inc/inc_catalog_options.php");
    ...
    $tags = GetTags($aid);
    include XAKINClude("templets/album_edit.htm");
    exit();
}
/*--------------------------------
function __save(){  }
...
else if($dopost=='save')
{
    ...
    //按照栏目设置，修改缩略图大小
    $type_param = $dsql->GetOne("SELECT ddimg_width, ddimg_height FROM `#@__arctype` WHERE id ='$typeid'");
    if(!empty($type_param['ddimg_width'])){
        $cfg_ddimg_width = $type_param['ddimg_width'];
    }
    if(!empty($type_param['ddimg_height'])){
        $cfg_ddimg_height = $type_param['ddimg_height'];
    }
    ...
    $imgurls = "{xak:pagestyle maxwidth='$maxwidth' pagepicnum='$pagepicnum' ddmaxwidth='$ddmaxwidth' row='$row' col='$col' value='$pagestyle'/}\r\n";
    $hasone = false;
    ...
    for($i=1; $i<=120; $i++)
    {
        ...
        if( isset(${'imgfile'.$i}) && is_uploaded_file(${'imgfile'.$i}) )
        {
            ...
            if(!is_array($imginfos))
            {
                ...
                $imgurls .= "{xak:img ddimg='$ddurl' text='$iinfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/xak:img}\r\n";
                continue;
            }
            ...
            $imgurls .= "{xak:img ddimg='$litpicname' text='$iinfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/xak:img}\r\n";
        }
        //没上传图片(只修改msg信息)
        else
        {
    ...
    if($formzip==1)
    {
        include_once(XAKINC."/zip.class.php");
        include_once(XAKADMIN."/file_class.php");
        $zipfile = $cfg_basedir.str_replace($cfg_mainsite,'',$zipfile);
        $tmpzipdir = XAKDATA.'/ziptmp/'.cn_substr(md5(ExecTime()),16);
        $ntime = time();
        if(file_exists($zipfile))
        {
            ...
            foreach($imgs as $imgold)
            {
                ...
                if(is_file($imgfile))
                {
                    ...
                    $info = '';
                    $imginfos = GetImageSize($imgfile,$info);
                    $imgurls .= "{xak:img ddimg='$litpicname' text='' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/xak:img}\r\n";
    ...
    if(is_array($_SESSION['bigfile_info']))
    {
        foreach($_SESSION['bigfile_info'] as $k=>$v)
        {
            ...
            $imgurls .= "{xak:img ddimg='$litpicname' text='$imginfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} $v {/xak:img}\r\n";
        }
    }
    ...
    $inadd_f = '';
    $inadd_v = '';
    if(!empty($xak_addonfields))
    {
        $addonfields = explode(';', $xak_addonfields);
        $inadd_f = '';
        $inadd_v = '';
