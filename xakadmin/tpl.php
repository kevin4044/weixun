...
if($action == 'edit' || $action == 'newfile')
{
    ...
    foreach($dtags as $tag)
    {
        ...
        fclose($fp);
        $helps[$tag] = explode('>>xak>>', $helpContent);
    }

    include XAKADMIN.'/templets/tpl_edit.htm';
    exit();
}
/*---------------------------
function save_tpl() { }
...
?".'>';
    ...
    else
    {
        ...
        $fp = fopen(XAKINC.'/taglib/'.$filename,'r');
        $democode = fread($fp, filesize(XAKINC.'/taglib/'.$filename));
        fclose($fp);
        $title = "修改标签";
    }
    include XAKADMIN.'/templets/tpl_edit_tag.htm';
    exit();
}
/*---------------------------
...
else if($action=='savetagfile')
{
    ...
    $content = stripslashes($content);
    $truefile = XAKINC.'/taglib/'.$filename;
    $fp = fopen($truefile, 'w');
    fwrite($fp, $content);
    fclose($fp);
    $msg = "
    <form name='form1' action='tag_test_action.php' target='blank' method='post'>
      <input type='hidden' name='dopost' value='make' />
        ...
        <textarea name='partcode' cols='150' rows='6' style='width:90%;'>{xak:{$tagname} }{/xak:{$tagname}}</textarea><br />
