...
class SplitWord
{
    ...
    function __construct($source_charset='utf-8', $target_charset='utf-8', $load_all=TRUE, $source='')
    {
        ...
        if(file_exists(XAKINC.'/'.$this->mainDicFile)) $this->isUnpacked = TRUE;
    ...
    */
    ...
    {
		$this->addonDicFile = XAKINC.'/'.$this->addonDicFile;
		$this->mainDicFile = XAKINC.'/'.$this->mainDicFile;
		$this->mainDicFileZip = XAKINC.'/'.$this->mainDicFileZip;
	...
	function InportDict( $targetfile )
     {
     	if(!ini_set('memory_limit', '128M'))
			exit('设置内存错误!');
     	require_once(XAKINC.'/zip.class.php');
     	$zip = new zip();
     	//echo $targetfile;
     	$unpackagefile = array_keys($zip->Extract($targetfile,XAKINC.'/data/'));
     	//exit();
     	$this->MakeDict(XAKINC.'/data/'.$unpackagefile[0]);
     	unlink(XAKINC.'/data/'.$unpackagefile[0]);
     	return true;
     }
}
