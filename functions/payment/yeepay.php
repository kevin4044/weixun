...
class yeepay
{
    ...
    var $return_url='/module/carbuyaction.php?dopost=return'; //返回处理地址
    ...
    */
    function GetCode($order, $payment)
    {
        ...
        //$p8_Url                        = $cfg_basehost."/module/carbuyaction.php?dopost=return&code=".$payment['code'];  
    ...
    */
    function respond()
    {
        ...
        require_once XAKDATA.'/payment/'.$code.'.php';
            ...
            if($r1_Code=="1")
            {
                ...
                if($r9_BType == "1" || $r9_BType == "3"){
                    if($ordertype == "goods"){ 
                        ...
                        return $msg = "<font color='red'>".$oldinf."</font><br> <a href='/'>返回主页</a> <a href='/user'>会员中心</a>";
                    }
                } else if ( $r9_BType == "2" ){
    ...
    function success_mem($order_sn,$pname,$product,$pid)
    {
        ...
        if($product=="card")
        {
            ...
            if(!is_array($row))
            {
                ...
                $dnum = $nrow['num'];
                $sql1 = "UPDATE `#@__member_list` SET `money`=money+'{$nrow['num']}' WHERE `mid`='".$this->mid."'";
        ...
        /* 改变会员订单状态_支付成功 */
            ...
            $exptime = $row['exptime'];
            /*计算原来升级剩余的天数*/
            $rs = $this->dsql->GetOne("SELECT uptime,exptime FROM `#@__member_list` WHERE mid='".$this->mid."'");
            ...
            //更新会员信息
            $sql1 =  " UPDATE `#@__member_list` SET rank='$rank',money=money+'{$memrank['money']}',
    ...
    function  log_result($word) {
        global $cfg_cmspath;
        $fp = fopen(dirname(__FILE__)."/../../config/payment/log.txt","a");
        flock($fp, LOCK_EX) ;
