 ...
 */
require_once(XAKINC.'/xakhttpdown.class.php');
require_once(XAKINC.'/image.func.php');
require_once(XAKINC.'/archives.func.php');
...
function GetCurContentAlbum($body, $rfurl, &$firstdd)
{
    ...
    require_once(XAKINC.'/xakcollection.func.php');
    ...
    foreach($img_array as $key=>$value)
    {
        ...
        if($rs)
        {
            ...
            $rsimg .= "{xak:img ddimg='$litpicname' text='' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/xak:img}\r\n";
        }
    }
    return $rsimg;
}
...
{
    global $envs, $typeid;
    require_once(XAKINC.'/arc.archives.class.php');
...
function PrintAutoFieldsAdd(&$fieldset, $loadtype='all')
{
    ...
    if(is_array($dtp->CTags))
    {
        foreach($dtp->CTags as $tid=>$ctag)
        {
            ...
            {
                $xak_addonfields .= ( $xak_addonfields=="" ? $ctag->GetName().",".$ctag->GetAtt('type') : ";".$ctag->GetName().",".$ctag->GetAtt('type') );
                echo  GetFormItemA($ctag);
            }
        }
    }
    echo "<input type='hidden' name='xak_addonfields' value=\"".$xak_addonfields."\">\r\n";
}
...
function AnalyseHtmlBody($body,&$description,&$litpic,&$keywords,$dtype='')
{
    ...
    if($dellink==1)
    {
        ...
        // 读取允许的超链接设置
        if(file_exists(XAKDATA."/admin/allowurl.txt"))
        {
            $allow_urls = array_merge($allow_urls, file(XAKDATA."/admin/allowurl.txt"));
        }
    ...
    if($autokey==1 && $keywords=='')
    {
        $subject = $title;
        $message = $body;
        include_once(XAKINC.'/splitword.class.php');
        $keywords = '';
