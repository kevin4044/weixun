...
class XakSql
{
    ...
    function getCol($sql)
    {
        ...
        {
            $arr = array();
            while ($row = mysql_fetch_row($this->result["col"]))
            {
                $arr[] = $row[0];
            }
...
EOT;
    ...
    {
        $errorTrackFile = dirname(__FILE__).'/../config/mysql_error_trace.inc';
        if( file_exists(dirname(__FILE__).'/../config/mysql_error_trace.php') )
        {
            @unlink(dirname(__FILE__).'/../config/mysql_error_trace.php');
        }
