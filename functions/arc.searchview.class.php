 ...
 */
require_once(XAKINC."/typelink.class.php");
require_once(XAKINC."/xaktag.class.php");
require_once(XAKINC."/splitword.class.php");
require_once(XAKINC."/taglib/hotwords.lib.php");
require_once(XAKINC."/taglib/channel.lib.php");
require_once(XAKINC."/taglib/rpad.lib.php");
require_once(XAKINC."/taglib/type.lib.php");
...
class SearchView
{
     ...
     * @return    string
     */
    function __construct($typeid,$keyword,$orderby,$achanneltype="all",$searchtype='',$starttime=0,$upagesize=20,$kwtype=1,$mid=0,$lang='cn',$diy_tmp='')
    {
        ...
        $this->dtp = new XakTagParse();
        $this->dtp->SetRefObj($this);
        $this->dtp->SetNameSpace("xak","{","}");
        $this->dtp2 = new XakTagParse();
        ...
        $this->lang = $lang;
		$this->diy_tmp = $diy_tmp;
		//模板路径
		$tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$GLOBALS['cfg_df_style'].$diy_tmp."/search.htm";
    ...
    //php4构造函数
    function SearchView($typeid,$keyword,$orderby,$achanneltype="all",$searchtype="",$starttime=0,$upagesize=20,$kwtype=1,$mid=0,$lang='cn',$diy_tmp='')
    {
        $this->__construct($typeid,$keyword,$orderby,$achanneltype,$searchtype,$starttime,$upagesize,$kwtype,$mid,$lang,$diy_tmp);
    }
    ...
    function Display()
    {
        foreach($this->dtp->CTags as $tagid=>$ctag)
        {
            ...
            else if($tagname=="rpad")
            {
                $this->dtp->Assign($tagid,lib_rpad($ctag,$this));
            }//End if
    ...
    function GetPageListDM($list_len)
    {
        ...
        {
            return "$lang_total 1 $lang_page/".$this->TotalResult.$lang_record;
        }
        if($this->TotalResult == 0)
        {
            return "$lang_total 0 $lang_page/".$this->TotalResult.$lang_record;
        }
        $purl = $this->GetCurUrl();
        ...
        $infos = "<li><span class=\"pageinfo\">$lang_find ".$this->TotalResult." $lang_record/$lang_show <b>{$totalpage}</b> $lang_page </span></li>\r\n";
		...
		if($this->lang=='en')
		{
			$geturl .= "&lang=".$this->lang;
			$hidenform .= "<input type='hidden' name='lang' value='".$this->lang."'>\r\n";
		}
		//加入自定义模板url
		if($this->diy_tmp!='')
		{
			$geturl .= "&diy_tmp=".$this->diy_tmp;
			$hidenform .= "<input type='hidden' name='diy_tmp' value='".$this->diy_tmp."'>\r\n";
		}
        ...
        if($this->PageNo != 1)
        {
            $prepage.="<li><a href='".$purl."PageNo=$prepagenum'>$lang_pre</a></li>\r\n";
            $indexpage="<li><a href='".$purl."PageNo=1'>$lang_index</a></li>\r\n";
        }
        else
        {
            $indexpage="<li><a href='javascript:void(0);'>$lang_index</a></li>\r\n";
        }
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
            $nextpage.="<li><a href='".$purl."PageNo=$nextpagenum'>$lang_next</a></li>\r\n";
            $endpage="<li><a href='".$purl."PageNo=$totalpage'>$lang_end</a></li>\r\n";
        }
        else
        {
            $endpage="<li><a href='javascript:void(0);'>$lang_end</a></li>\r\n";
        }
        ...
        {
            $plist.="<li><input type='text' name='PageNo' style='width:28px;height:14px' value='".$this->PageNo."' /></li>\r\n";
            $plist.="<li><input type='submit' name='plistgo' value='GO' style='width:30px;height:22px;font-size:9pt' /></li>\r\n";
        }
        $plist .= "</form>\r\n";
        return $plist;
    }
