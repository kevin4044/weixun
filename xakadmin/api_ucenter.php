...
if(file_exists(XAKROOT.'/uc_client/client.php'))
{
	if(!defined('UC_API')) define('UC_API', '');
	include_once XAKROOT.'/uc_client/client.php';
}
else
{
...
class api_ucenter
{
	...
	function __construct($ac = '')
	{
		...
		$this->dtp = new XakTemplate();
		$this->config = XAKINC.'/common.inc.php';
		$this->$action();
	}
	...
	function uc_install()
	{
		...
		else
		{
			...
			{
				ShowMsg('uc服务端编码与系统编码不一致!要求您的uc服务端编码为:gbk编码.',-1);
				exit();	
			}
			//标签应用模板
	...
	function uc_show()
	{
		...
		if(!defined('UC_APPID'))
		{
			$this->dtp->LoadTemplate(XAKADMIN.'/templets/api_ucenter_install.htm');
		}
		else
		{
			$uc_api_open = false;			
			...
			$this->dtp->LoadTemplate(XAKADMIN.'/templets/api_ucenter_edit.htm');
		}
		$this->dtp->Display();
		exit();
	}
