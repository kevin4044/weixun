...
function editTitle(aid){
    ...
    var myajax = new XakAjax(show,false,false,"","","");
    myajax.SendGet2("catalog_edit.php?dopost=time&id="+aid);
    XakXHTTP = null;
}

function $Xak(id){ return document.getElementById(id)}
function AlertMsg(title,id){
	...
	msgObj.style.top = "60px";
	msgObj.style.left = "150px";
	...
	$Xak("msgdiv").appendChild(thObj);
	$Xak("msgdiv").appendChild(bodyObj);
	editTitle(id);
}
function CloseMsg(){
	//移除对象
	document.body.removeChild($Xak("maskdiv")); 
	$Xak("msgdiv").removeChild($Xak("msgth")); 
	$Xak("msgdiv").removeChild($Xak("msgbody")); 
	document.body.removeChild($Xak("msgdiv")); 
}
//拖动窗口
var ie = document.all;   
