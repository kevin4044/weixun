...
if($dopost=="add"){
  ...
  $dsql->Execute();
	...
	require_once(XAKADMIN."/templets/mail_title_add.htm");	
}elseif($dopost=="edit"){
	$dsql->SetQuery("SELECT id,typename FROM `#@__mail_type` ORDER BY id");
  $dsql->Execute();
	$btypes = Array();
  ...
  require_once(XAKADMIN."/templets/mail_title_edit.htm");	
  ...
  ShowMsg("删除期刊成功！","mail_title.php");
	...
	$dlist->SetTemplet(XAKADMIN."/templets/mail_title_main.htm");
	$dlist->SetSource($sql);
	$dlist->display();
}
