...
$dsql = $dsqli = $db = new XakSqli(FALSE);
/**
 * MySQLi数据库类
 */
class XakSqli
{
    ...
    function XakSql($pconnect=FALSE,$nconnect=TRUE)
    {
    ...
    //用指定参数初始数据库信息
    function SetSource($host,$username,$pwd,$dbname,$dbprefix="xak_")
    {
        $this->dbHost = $host;
    ...
    function getCol($sql)
    {
        ...
        {
            $arr = array();
            while ($row = mysqli_fetch_row($this->result["col"]))
            {
                $arr[] = $row[0];
            }
	...
	{
		$RecordLogFile = dirname(__FILE__).'/../config/mysqli_record_log.inc';
		$url = $this->GetCurUrl();
		$savemsg = <<<EOT
...
EOT;
    ...
    {
        $errorTrackFile = dirname(__FILE__).'/../config/mysqli_error_trace.inc';
        if( file_exists(dirname(__FILE__).'/../config/mysqli_error_trace.php') )
        {
            @unlink(dirname(__FILE__).'/../config/mysqli_error_trace.php');
        }
        $emsg = 'Mysql Error Warning!';
...
if (!function_exists('CheckSql'))
{
    function CheckSql($db_string,$querytype='select')
    {
        ...
        $old_pos = 0;
        $pos = -1;
        $log_file = XAKINC.'/../config/'.md5($cfg_cookie_encode).'_safe.txt';
        $userIP = GetIP();
        $getUrl = GetCurUrl();
