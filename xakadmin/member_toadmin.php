...
if($dopost == "toadmin")
{
    ...
    {
        $inputpwd = ",pwd";
		$inputpwdv = ",'".PassJoin($pwd)."'";
        $pwdm = ",pwd='".PassJoin($pwd)."'";
        //$inputpwdv = ",'".substr(md5($pwd), 5, 20)."'";
        //$pwdm = ",pwd='".md5($pwd)."'";
    }else{
        $row = $dsql->GetOne("SELECT * FROM `#@__member_list` WHERE mid='$id'");
        $password = $row['pwd'];
        $inputpwd = ",pwd";
    ...
    if($id!=1)
    {
        $query = "INSERT INTO `#@__admin_list`(id,usertype,userid$inputpwd,uname,typeid,tname,email)
                    VALUES('$id','$usertype','$userid'$inputpwdv,'$uname','$typeid','$tname','$email')";
    }
    else
    {
        $query = "INSERT INTO `#@__admin_list`(id,userid$inputpwd,uname,typeid,tname,email)
                    VALUES('$id','$userid'$inputpwdv,'$uname','$typeid','$tname','$email')";
    }
    $dsql->ExecuteNoneQuery($query);
    $query = "UPDATE `#@__member_list` SET rank='100',uname='$uname',matt='10',email='$email'$pwdm WHERE mid='$id'";
    $dsql->ExecuteNoneQuery($query);
...
$row = $dsql->GetOne("SELECT * FROM `#@__member_list` WHERE mid='$id'");
include XAKINClude('templets/member_toadmin.htm');