 ...
 * 采集类
 *
 * @version        $Id: xakcollection.class.php 1 20:20 2010年7月7日
 */

require_once(XAKINC."/xakcollection.func.php"); //采集扩展函数
require_once(XAKINC."/image.func.php");
require_once(XAKINC."/xakhtml2.class.php");
@set_time_limit(0);
...
class XakCollection
{
    ...
    function __construct()
    {
        ...
        $this->cHttpDown = new XakHttpDown();
        $this->cXakHtml = new XakHtml2();
    }

    function XakCollection()
    {
        $this->__construct();
    }
    ...
    function GetSpPage($dourl, $noteid, $html, $step=0)
    {
        ...
        {
            $this->tmpUnitValue .= $this->GetHtmlArea('[内容]', $sarr['match'], $html);
            $this->cXakHtml->GetLinkType = "link";
            $this->cXakHtml->SetSource($linkareaHtml, $dourl, 'link');
            foreach($this->cXakHtml->Links as $k=>$t)
            {
                $k = $this->cXakHtml->FillUrl($k);
                if($k==$dourl)
                {
                    continue;
                }
        ...
        else if ($this->artNotes["sptype"] == 'diyrule')
        {
            ...
            for($i=$this->spNotes['srul']; $i <= $this->spNotes['erul']; $i++)
            {
                ...
                $tempurl = $this->cXakHtml->FillUrl($tempurl);
    ...
    function GetPageFields($dourl, $needDown, $litpic='')
    {
        ...
        if(isset($inarr[1]))
        {
            ...
            $artitem .= "{xak:field name='keywords'}".$keywords."{/xak:field}\r\n";
        }
        else
        {
            $artitem .= "{xak:field name='keywords'}{/xak:field}\r\n";
        }
        ...
        if(isset($inarr[1]))
        {
            ...
            $artitem .= "{xak:field name='description'}".$description."{/xak:field}\r\n";
        }
        else
        {
            $artitem .= "{xak:field name='description'}{/xak:field}\r\n";
        }
        ...
        foreach($this->artNotes as $k=>$sarr)
        {
            ...
            else
            {
                $v = preg_replace("#(　)$#", '', $v);
                $v = preg_replace("#[\r\n\t ]{1,}$#", '', $v);
                $artitem .= "{xak:field name='$k'}$v{/xak:field}\r\n";
            }
        }//End Foreach
        ...
        {
            $artitem .= "{xak:field name='litpic'}".$this->DownMedia($litpic,'img',TRUE)."{/xak:field}\r\n";
        }
        else
        {
            $artitem .= "{xak:field name='litpic'}".$this->breImage."{/xak:field}\r\n";
        }
        return $artitem;
    }
    ...
    {
        $this->cXakHtml->SetSource($html,$url,'media');

        //下载标记里的图片和flash
        foreach($this->cXakHtml->Medias as $k=>$v)
        {
            $furl = $this->cXakHtml->FillUrl($k);
        ...
        //下载超链接里的图片
        foreach($this->cXakHtml->Links as $v=>$k)
        {
    ...
    {
        $this->cXakHtml->SetSource($html, $dourl, 'media');
        foreach($this->cXakHtml->Medias as $k=>$v)
        {
            $k = trim($k);
            $okurl = $this->cXakHtml->FillUrl($k);
            $html = str_replace($k, $okurl, $html);
        }
        return $html;
    }
        ...
        else
        {
            ...
            foreach($this->lists['url'] as $k=>$cururls)
            {
                ...
                if($moviePostion > $glstart)
                {
                    ...
                    $this->cXakHtml->SetSource($html, $cururl, 'link');
                    $lk = 0;
                    foreach($this->cXakHtml->Links as $k=>$v)
                    {
                        if($this->lists['nothas']!='')
                        {
    ...
    {
        $XakMeValue = $fvalue;
        $phpcode = preg_replace("#'@me'|\"@me\"|@me#isU", '$XakMeValue', $phpcode);
        if(preg_match("#@body#i", $phpcode))
        {
            $XakBodyValue = $this->tmpHtml;
            $phpcode = preg_replace("#'@body'|\"@body\"|@body#isU", '$XakBodyValue', $phpcode);
        }
        if(preg_match("#@litpic#i", $phpcode))
        {
            $XakLitPicValue = $this->breImage;
            $phpcode = preg_replace("#'@litpic'|\"@litpic\"|@litpic#isU", '$XakLitPicValue', $phpcode);
        }
        eval($phpcode.";");
        return $XakMeValue;
    }
