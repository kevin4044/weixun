...
{
	require_once XAKINC.'/request.class.php';
	$request = new Request();
	$request->Init();
	...
	if(!empty($logoimg))
	{
		$request->MoveUploadFile('logoimg', XAKROOT.$cfg_medias_dir.'/flink/'.$request->GetFileInfo('logoimg', 'name'));
		$logo = $cfg_cmspath.$cfg_medias_dir.'/flink/'.$request->GetFileInfo('logoimg', 'name');
	}
...
include XAKINClude('templets/friendlink_edit.htm');