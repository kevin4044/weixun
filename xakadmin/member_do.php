...
if($dopost=="delmember")
{
    ...
    if($fmdo=='yes')
    {
        ...
        {
            //删除用户信息
            $row = $dsql->GetOne("SELECT * FROM `#@__member_list` WHERE mid='$id' LIMIT 1 ");
            $rs = 0;
            if($row['matt'] == 10)
            {
                $nrow = $dsql->GetOne("SELECT * FROM `#@__admin_list` WHERE id='$id' LIMIT 1 ");
                //已经删除关连的管理员帐号
                if(!is_array($nrow)) $rs = $dsql->ExecuteNoneQuery2("DELETE FROM `#@__member_list` WHERE mid='$id' LIMIT 1");
            }
            else
            {
                $rs = $dsql->ExecuteNoneQuery2("DELETE FROM `#@__member_list` WHERE mid='$id' LIMIT 1");
            }
            if($rs > 0)
            {
    ...
    if($fmdo=='yes')
    {
        ...
        if(!empty($id))
        {
            ...
            $rs = $dsql->ExecuteNoneQuery2("DELETE FROM `#@__member_list` WHERE mid IN (".str_replace("`",",",$id).") And matt<>10 ");    
            if($rs > 0)
            {
...
else if ($dopost=="recommend")
{
    ...
    if($matt==0)
    {
        $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` SET matt=1 WHERE mid='$id' AND matt<>10 LIMIT 1");
        ShowMsg("成功设置一个会员推荐！",$ENV_GOBACK_URL);
        exit();
    }
    else
    {
        $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` SET matt=0 WHERE mid='$id' AND matt<>10 LIMIT 1");
...
{
    CheckPurview('member_Edit');
    if(!isset($_POST['id'])) exit('Warning...');
	
    $pwdsql = empty($pwd) ? '' : ",pwd='".PassJoin($pwd)."'";
    ...
    $query = "UPDATE `#@__member_list` SET
            email = '$email',
            uname = '$uname',
            sex = '$sex',
            matt = '$matt',
    ...
    #api{{
    if(defined('UC_API') && @include_once XAKROOT.'/api/uc.func.php')
    {
        $row = $dsql->GetOne("SELECT `scores`,`userid` FROM `#@__member_list` WHERE `mid`='$id' AND `matt`<>10");
...
{
    CheckPurview('member_Edit');
    PutCookie('XakUserID',$id,1800);
    PutCookie('XakLoginTime',time(),1800);
