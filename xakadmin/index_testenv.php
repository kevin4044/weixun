...
if(!function_exists('TestExecuteable'))
{
	...
	function TestExecuteable($d='.', $siteuRL='', $rootDir='') {
		...
		$testStr .= chr(0x3F).'>';
		$reval = false;
		if(empty($rootDir)) $rootDir = XAKROOT;
		if (TestWriteable($d)) 
		{
...
if(!function_exists('TestAdminPWD'))
{
	...
	function TestAdminPWD() 
	{
		global $dsql;
		// 查询栏目表确定栏目所在的目录
		$sql = "SELECT usertype,userid,pwd FROM `#@__admin_list` WHERE `userid`='admin'";
		$row = $dsql->GetOne($sql);
		if(is_array($row))
		{
	...
	function IsWritable($pathfile) {
		...
		if ($isDir) {
			if (is_dir($pathfile)) {
				...
				$pathfile = $pathfile.'xak_'.uniqid(mt_rand()).'.tmp';
...
// 检测权限
$safeMsg = array();
if(TestExecuteable(XAKROOT.'/config',$cfg_basehost) || TestExecuteable(XAKROOT.'/uploads',$cfg_basehost))
{
	$safeMsg[] = '目前data、uploads有执行.php权限，非常危险，需要立即取消目录的执行权限！
	<a href="testenv.php" title="全面检测"><img src="images/btn_fullscan.gif" /></a>';
}

if(IsWritable(XAKDATA.'/common.inc.php'))
{
...
if(PostHost($cfg_basehost.'/config/admin/ver.txt') === @file_get_contents(XAKDATA.'/admin/ver.txt'))
{
	$safeMsg[] = '<font color="blue">强烈建议将data目录搬移到Web根目录以外；';
}
?>
<?php
if(count($safeMsg) > 0)
