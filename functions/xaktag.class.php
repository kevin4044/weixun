 ...
 * 模板类
 *
 * @version        $Id: xaktag.class.php 1 10:33 2010年7月6日
 */

/**
 * class XakTag 标记的数据结构描述
 * function c___XakTag();
 *
 * @package          XakTag
 */ 
...
// ------------------------------------------------------------------------
    ...
    function LoadCache($filename)
    {
        ...
        $cachedir = XAKROOT.$cfg_tplcache_dir;
        ...
        if( isset($z) && is_array($z) )
        {
            foreach($z as $k=>$v)
            {
                $this->Count++;
                $ctag = new XakTag();
                $ctag->CAttribute = new XakAttribute();
    ...
    function LoadSource($str)
    {
        ...
        */
        //优化模板字符串存取读取方式
        $this->taghashfile = $filename = XAKDATA.'/tplcache/'.md5($str).'.inc';
        if( !is_file($filename) )
        {
    ...
    function RunPHP(&$refObj, $i)
    {
        ...
        {
            $XakMeValue = $this->CTags[$i]->TagValue;
            $phpcode = $refObj->GetInnerText();
        }
        $phpcode = preg_replace("/'@me'|\"@me\"|@me/i", '$XakMeValue', $phpcode);
        @eval($phpcode); //or die("<xmp>$phpcode</xmp>");

        $this->CTags[$i]->TagValue = $XakMeValue;
        $this->CTags[$i]->IsReplace = TRUE;
    }
    ...
    {
        $fp = @fopen($filename,"w") or die("XakTag Engine Create File False");
        ...
        for($i=0; $i < $sourceLen; $i++)
        {
            ...
            */
            ...
            if($tTagName!='')
            {
                ...
                {
                    //if '/}' more near '{xak:'、'{/xak:' , end tag is '/}', else is '{/xak:'
                    if($e1 < $e2 &&  $e1 < $e3 )
                    {
                        $endPos = $e1;
                        $elen = $endPos + strlen($eTagEndWord);
    ...
    {
        $XakFieldValue = $fieldvalue;
        $functionname = str_replace("{\"","[\"",$functionname);
        $functionname = str_replace("\"}","\"]",$functionname);
        $functionname = preg_replace("/'@me'|\"@me\"|@me/i",'$XakFieldValue',$functionname);
        $functionname = "\$XakFieldValue = ".$functionname;
        @eval($functionname.";"); //or die("<xmp>$functionname</xmp>");
        if(empty($XakFieldValue))
        {
            return '';
        }
        else
        {
            return $XakFieldValue;
        }
    }
    ...
    function IncludeFile($filename, $ismake='no')
    {
        ...
        if( file_exists(XAKROOT."/templets/".$filename) )
        {
            $okfile = XAKROOT."/templets/".$filename;
        }
        else if(file_exists(XAKROOT.'/templets/'.$cfg_df_style.'/'.$filename) )
        {
            $okfile = XAKROOT.'/templets/'.$cfg_df_style.'/'.$filename;
        }
        else
        {
        ...
        if($ismake!="no")
        {
            require_once(XAKINC."/channelunit.func.php");
            $dtp = new XakTagParse();
...
class XakAttributeParse
{
    ...
    {
        $this->cAttributes = new XakAttribute();
        $strLen = 0;
