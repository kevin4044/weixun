...
if($action=='')
{
    require_once(XAKADMIN."/templets/makehtml_all.htm");
    exit();
}
else if($action=='make')
{
    ...
    {
        include_once(XAKADMIN."/makehtml_archives_action.php");
        exit();
    }
    //更新主页
    /*-------------------------
    ...
    if($step==3)
    {
        include_once(XAKINC."/arc.partview.class.php");
        $pv = new PartView();
        $row = $pv->dsql->GetOne("SELECT * FROM `#@__homepageset` ");
		$templet = str_replace("{style}", $cfg_df_style,$row['templet']);
		$homeFile = XAKADMIN.'/'.$row['position'];
