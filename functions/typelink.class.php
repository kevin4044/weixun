...
class TypeLink
{
    ...
    //获得某类目的链接列表 如：类目一>>类目二>> 这样的形式
    //islink 表示返回的列表是否带连接
    function GetPositionLink2($islink=true,$currID)
    {
        $indexpage = "<a href='".$this->indexUrl."'>".$this->indexName."</a>";
        if($this->valuePosition!="" && $islink)
        {
        ...
        else
        {
            ...
            {
                $this->valuePosition = $this->GetOneTypeLink($this->TypeInfos);
                if($this->TypeInfos['reid']!=$currID)
                {
                    //调用递归逻辑
                    $this->LogicGetPosition($this->TypeInfos['reid'],true,1);
                }
                $this->valuePosition = $indexpage.$this->SplitSymbol.$this->valuePosition;
                return $this->valuePosition;
            }
            else
            {
                ...
                {
                    //调用递归逻辑
                    $this->LogicGetPosition($this->TypeInfos['reid'],false,1);
                }
                return $this->valuePositionName;
            }
        }
    ...
    //获得某类目的链接列表，递归逻辑部分
    function LogicGetPosition($id,$islink,$topid = 0)
    {
    ...
    /**
     *  获得与该类相关的类目，本函数应用于模板标记{xak:channel}{/xak:channel}中
