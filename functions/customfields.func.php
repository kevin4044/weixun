...
function GetFormItem($ctag, $admintype='admin')
{
    ...
    else if($fieldType=='stepselect')
    {
            ...
            $cmspath = ( (empty($cfg_cmspath) || !preg_match('/[/$]/', $cfg_cmspath)) ? $cfg_cmspath.'/' : $cfg_cmspath );//2011.08.04 根据论坛反馈修正联动模型 （by：的鱼）
            $myformItem = '';
            ...
            $myformItem .= "<script language='javascript' type='text/javascript' src='{$cmspath}config/enums/{$fieldname}.js'></script>\r\n";
    ...
    {
        $innertext = "<textarea name='$fieldname' id='$fieldname' style='width:90%;height:80px;'></textarea>\r\n";
    }
...
function GetFieldValue($dvalue, $dtype, $aid=0, $job='add', $addvar='', $admintype='admin', $fieldname='')
{
    ...
    {
            $ipath = $cfg_cmspath."/config/textdata";
            $tpath = ceil($aid / 5000);
    ...
    else if($dtype=='img' || $dtype=='imgfile')
    {
        ...
        if(preg_match("/^http:\/\//i", $iurl) && $GLOBALS['cfg_isUrlOpen'])
        {
            ...
            if($GLOBALS['cfg_isUrlOpen'])
            {
                ...
                if(is_array($reimgs))
                {
                    ...
                    {
                        $imgurl = "{xak:img text='' width='".$reimgs[1]."' height='".$reimgs[2]."'} ".$reimgs[0]." {/xak:img}";
                    }
                }
            }
            else
            {
                ...
                {
                    $imgurl = "{xak:img text='' width='' height=''} ".$iurl." {/xak:img}";
                }
            }
        }
        else if($iurl != '')
        {
            ...
            if(is_file($imgfile))
            {
                ...
                {
                    $imgurl = "{xak:img text='' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/xak:img}";
                }
            }
        }
