<?php
if(!defined('XAKINC')) exit("Warning...");
/**
 * 文档类
 *
...
require_once(XAKINC."/channelunit.class.php");
require_once(XAKINC."/downmix.inc.php");
require_once(XAKINC.'/ftp.class.php');
...
class Archives
{
    ...
    {
        //读取附加表信息，并把附加表的资料经过编译处理后导入到$this->Fields中，以方便在模板中用 {xak:field name='fieldname' /} 标记统一调用
        if($this->ChannelUnit->ChannelInfos['addtable']!='')
        {
        ...
        if (isset($this->Fields['litpic']))
        {
            ...
            //模板里直接使用{xak:field name='image'/}获取缩略图
            $this->Fields['image'] = (!preg_match('/jpg|gif|png/i', $this->Fields['picname']) ? '' : "<img src='{$this->Fields['picname']}' />");
        }
		
		//2013-05-28 by digua
		//增加标题颜色
		if(isset($this->Fields['color']) && !empty($this->Fields['color']))
		{
			$this->Fields['color_title'] = '<font color="'.$this->Fields['color'].'">'.$this->Fields['title'].'</font>';
		}
		else
		{
			$this->Fields['color_title'] = $this->Fields['title'];
		}
        ...
        if (isset($this->Fields['voteid']) && !empty($this->Fields['voteid']))
        {
            ...
            $this->Fields['vote'] = "<script language='javascript' src='{$GLOBALS['cfg_cmspath']}/config/vote/vote_{$voteid}.js'></script>";
            if ($GLOBALS['cfg_multi_site'] == 'Y')
            {
                $this->Fields['vote'] = "<script language='javascript' src='{$GLOBALS['cfg_mainsite']}/config/vote/vote_{$voteid}.js'></script>";
            }
        }
        ...
        else
        {
            for($i=1;$i<=$this->TotalPage;$i++)
            {
                ...
                if($cfg_remote_site=='Y' && $isremote == 1)
                {
                    ...
                    //分析远程文件路径
                    $remotefile = str_replace(XAKROOT, '', $TRUEfilename);
    ...
    function ParseDMFields($pageNo, $ismake=1)
    {
        ...
        if(is_array($this->dtp->CTags))
        {
            foreach($this->dtp->CTags as $i=>$ctag)
            {
                ...
                {
					$plang = trim($ctag->GetAtt("sitelang"));
                    if($ismake==0)
                    {
                        $this->dtp->Assign($i,$this->GetPagebreakDM($this->TotalPage,$this->NowPage,$this->ArcID,$plang));
                    }
                    else
                    {
                        $this->dtp->Assign($i,$this->GetPagebreak($this->TotalPage,$this->NowPage,$this->ArcID,$plang));
                    }
                }
    ...
    function GetPreNext($gtype='', $site_lang)
    {
    	...
    	$str_pre      = 'Preview';
    	$str_next     = 'Next';
    	$str_pre_words   = 'Preview：Null';
    	$str_next_words   = 'Next：Null';
		}
        $rs = '';
     ...
     * @return    string
     */
    function GetPagebreakDM($totalPage, $nowPage, $aid,$plang)
    {
		//添加在这里
   		if($plang == 'cn' || $plang == '')
   		{
        ...
        $PageList = "<li><a>".$lang_total.$totalPage.$lang_page.": </a></li>";
        $nPage = $nowPage-1;
        $lPage = $nowPage+1;
        if($nowPage==1)
        {
            $PageList.="<li><a href='#'>".$lang_pre."</a></li>";
        }
        else
        {
            if($nPage==1)
            {
                $PageList.="<li><a href='view.php?aid=$aid'>".$lang_pre."</a></li>";
                if($cfg_rewrite == 'Y')
                {
            ...
            {
                $PageList.="<li><a href='view.php?aid=$aid&pageno=$nPage'>".$lang_pre."</a></li>";
                if($cfg_rewrite == 'Y')
                {
        ...
        if($lPage <= $totalPage)
        {
            $PageList.="<li><a href='view.php?aid=$aid&pageno=$lPage'>".$lang_next."</a></li>";
            if($cfg_rewrite == 'Y')
            {
                $PageList = str_replace(".php?aid=", "-", $PageList);
                $PageList =  preg_replace("#&pageno=(\d+)#i", '-\\1.html', $PageList);
        ...
        {
            $PageList.= "<li><a href='#'>".$lang_next."</a></li>";
        }
        return $PageList;
    }
     ...
     * @return    string
     */
    function GetPagebreak($totalPage, $nowPage, $aid ,$plang)
    {
		//添加在这里
   		if($plang == 'cn' || $plang == '')
   		{
        ...
        else
        {
            if($nPage==1)
            {
                $PageList.="<li><a href='".$this->NameFirst.".".$this->ShortName."'>".$lang_pre."</a></li>";
            }
            else
            {
                $PageList.="<li><a href='".$this->NameFirst."_".$nPage.".".$this->ShortName."'>".$lang_pre."</a></li>";
            }
        }
        for($i=1;$i<=$totalPage;$i++)
        {
        ...
        if($lPage <= $totalPage)
        {
            $PageList.="<li><a href='".$this->NameFirst."_".$lPage.".".$this->ShortName."'>".$lang_next."</a></li>";
        }
        else
        {
            $PageList.= "<li><a href='#'>".$lang_next."</a></li>";
    ...
    function GetPageTitlesDM($styleName, $pageNo)
    {
        ...
        {
            $revalue = "<select id='xakpagetitles' onchange='location.href=this.options[this.selectedIndex].value;'>\r\n";
            foreach($this->SplitTitles as $k=>$v)
            {
                if($i==1)
                {
