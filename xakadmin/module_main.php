...
require_once(dirname(__FILE__)."/../functions/xakmodule.class.php");
...
{
    global $dsql;
    $configfile = XAKDATA.'/config.cache.inc.php';
...
else if($action=='setup')
{
    ...
    $prvdir .= "<tr style='background:#F9FCFD'><th width='270'>目录</td><th align='center'>可写</td></tr>\r\n";
  ...
  <tr>
    <td height='30' class='dtb' bgcolor='#F9FCFD' colspan='2'>
    <b>注意事项：</b>
...
else if($action=='setupstart')
{
    ...
    {
        include(XAKDATA.'/module/'.$filename);
        exit();
    }
    //系统自动安装
    else
...
else if($action=='uninstallok')
{
    ...
    {
        include(XAKDATA."/module/{$hash}-uninstall.php");
        @unlink(XAKDATA."/module/{$hash}-uninstall.php");
        exit();
    }
    else
    {
...
else if($action=='view')
{
    ...
    if(file_exists(XAKDATA."/module/{$hash}-readme.php")) 
    {
