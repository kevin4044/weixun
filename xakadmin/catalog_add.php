...
if($dopost=='quick')
{
    ...
    include XAKINClude('templets/catalog_add_quick.htm');
    exit();
}
/*---------------------
...
else if($dopost=='savequick')
{
    ...
    tempindex,templist,temparticle,modname,namerule,namerule2,ispart,corank,description,keywords,seotitle,moresite,siteurl,sitepath,ishidden,`cross`,`crossid`,`content`,`smalltypes`,`open_new`)
    VALUES('~reid~','~topid~','~rank~','~typename~','~typedir~','$isdefault','$defaultname','$issend','$channeltype',
    '$tempindex','$templist','$temparticle','default','$namerule','$namerule2','0','0','','','','0','','','0','0','0','','','0')";
    ...
    if (empty($savetype))
    {
        foreach($_POST as $k=>$v)
        {
			...
			//增加栏目目录功能
			$topname = trim(${'topname'.$k});
			if(empty($topname)){
				$toptypedir = GetPinyin(stripslashes($toptypename));
			}
			else
			{
				$toptypedir = stripslashes($topname);
			}
        ...
        $row = $dsql->GetOne("SELECT `typedir`,`topid` FROM `#@__arctype` WHERE `id`={$reid}");
		if($row['topid']!=0)
		{
			$topid = $row['topid'];
		}
        ...
        foreach($_POST as $k=>$v)
        {
            ...
            $sql = str_replace('~topid~', $topid, $sql);
            $sql = str_replace('~rank~', $rank, $sql);
            $sql = str_replace('~typename~', $toptypename, $sql);
            $sql = str_replace('~typedir~', $toptypedir, $sql);
            $dsql->ExecuteNoneQuery($sql);
...
else if($dopost=='save')
{
    ...
    ispart,corank,description,keywords,seotitle,moresite,siteurl,sitepath,ishidden,`cross`,`crossid`,`content`,`smalltypes`,typepic,open_new,ddimg_width,ddimg_height)
...
if($dopost=='')
{
    ...
    if($id>0)
    {
        ...
        $ddimg_width = $myrow['ddimg_width'];
        $ddimg_height = $myrow['ddimg_height'];
    }
...
include XAKINClude('templets/catalog_add.htm');