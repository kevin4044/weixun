...
if ( ! function_exists('WaterImg'))
{
    function WaterImg($srcFile, $fromGo='up')
    {
        ...
        include(XAKDATA.'/mark/inc_photowatermark_config.php');
        require_once(XAKINC.'/image.class.php');
        ...
        if($fromGo=='down' && $photo_markdown=='0')
        {
         ...
         $TRUEMarkimg = XAKDATA.'/mark/'.$photo_markimg;
        ...
        if($photo_marktype == '2')
        {
            if(file_exists(XAKINC.'/data/fonts/elephant.ttf'))
            {
                $cfg_watermarktext['fontpath'] =  XAKINC.'/data/fonts/elephant.ttf';
            }
            else
            {
                return ;
