...
if ( ! function_exists('GetTypeUrl'))
{
    function GetTypeUrl($typeid,$typedir,$isdefault,$defaultname,$ispart,$namerule2,$moresite=0,$siteurl='',$sitepath='')
    {
        ...
        {
			global $dsql;
			$my_rows = $dsql->GetOne("SELECT * FROM #@__arctype where reid='$typeid' AND ishidden ='0' ORDER BY sortrank ASC");
		}
        ...
        //链接到第一个子分类
   	   else if($isdefault==2 && is_array($my_rows))
		{
			$reurl = GetOneTypeUrlA($my_rows);
		}
        else if($ispart==2)
        {
            //跳转网址
...
if ( ! function_exists('GetTopids'))
{
    ...
    {
		if(isset($GLOBALS['pTypeArrays']))
		{
			unset($GLOBALS['pTypeArrays']);
		}
    ...
    foreach($dtp->CTags as $tagid=>$ctag)
    {
        ...
        if(in_array($tagname,$alltags))
        {
           if(in_array($tagname, $disable_tags))
            {
                echo 'Error:Tag disabled:"'.$tagname;
                return FALSE;
            }
            if (DEBUG_LEVEL==TRUE) {
                $ttt1 = ExecTime();
            }
            $filename = XAKINC.'/taglib/'.$tagname.'.lib.php';
...
function GetBookUrl($bid,$title,$gdir=0)
{
    global $cfg_cmspath;
    $bookurl = $gdir==1 ?
    "{$cfg_cmspath}/book/".XakID2Dir($bid) : "{$cfg_cmspath}/book/".XakID2Dir($bid).'/'.GetPinyin($title).'-'.$bid.'.html';
    return $bookurl;
}
...
function GetCacheBlock($cacheid)
{
    ...
    $cachefile = XAKDATA.'/cache/'.$cacheid.'.inc';
