...
require(XAKINC.'/image.func.php');
require(XAKINC.'/xaktag.class.php');
...
if(empty($dopost))
{
    ...
    $myMoveFile = XAKDATA.'/admin/move-'.$cuserLogin->getUserID().'.txt';
    if(file_exists($myMoveFile))
    {
        ...
        include XAKINClude('templets/index_body_move.htm');
    }else{  
        include XAKINClude('templets/index_body.htm');
    }
    exit();
}
/*-----------------------
...
else if($dopost=='addnew')
{
    ...
    $myIcoFileTrue = XAKDATA.'/admin/quickmenu-'.$cuserLogin->getUserID().'.txt';
...
{   
    ...
    $myIcoFileTrue = XAKDATA.'/admin/move-'.$cuserLogin->getUserID().'.txt';
    $fp = fopen($myIcoFileTrue,'w');
    fwrite($fp,$movedata);
    fclose($fp);
}
...
{
    $query = " SELECT COUNT(*) AS dd FROM `#@__member_list` ";
...
} else if($dopost=='setskin')
{
	...
	$skinconfig = XAKDATA.'/admin/skin.txt';
