...
if($dopost=='add')
{
    ...
    $row = $dsql->GetOne("SELECT COUNT(*) AS dd FROM `#@__member_list` WHERE userid LIKE '$userid' ");
    if($row['dd']>0)
    {
        ShowMsg('用户名已存在！','-1');
        exit();
    ...
    //关连前台会员帐号
    $adminquery = "INSERT INTO `#@__member_list` (`mtype`,`userid`,`pwd`,`uname`,`sex`,`rank`,`money`,`email`,
    ...
    //后台管理员
    $inquery = "INSERT INTO `#@__admin_list`(id,usertype,userid,pwd,uname,typeid,tname,email)
...
include XAKINClude('templets/sys_admin_user_add.htm');