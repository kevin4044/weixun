...
if($dopost=='save')
{
    ...
    $query = "UPDATE `#@__member_list` SET `face` = '$face' WHERE mid='{$cfg_ml->M_ID}' ";
...
else if($dopost=='delold')
{
    ...
    $query = "UPDATE `#@__member_list` SET `face` = '' WHERE mid='{$cfg_ml->M_ID}' ";
    $dsql->ExecuteNoneQuery($query);
    // 清除缓存
    $cfg_ml->DelCache($cfg_ml->M_ID);
...
include(XAKMEMBER."/templets/edit_face.htm");
exit();
?>