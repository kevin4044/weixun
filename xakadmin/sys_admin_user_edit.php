...
if($dopost=='saveedit')
{
    ...
    if($pwd != '')
    {
        $pwdm = ",pwd='".PassJoin($pwd)."'";
        $pwd = ",pwd='".PassJoin($pwd)."'";
    ...
    if($id!=1){
        $query = "UPDATE `#@__admin_list` SET uname='$uname',usertype='$usertype',tname='$tname',email='$email',typeid='$typeid' $pwd WHERE id='$id'";
    }else{
        $query = "UPDATE `#@__admin_list` SET uname='$uname',tname='$tname',email='$email',typeid='$typeid' $pwd WHERE id='$id'";
    }
    $dsql->ExecuteNoneQuery($query);
    $query = "UPDATE `#@__member_list` SET uname='$uname',email='$email'$pwdm WHERE mid='$id'";
    $dsql->ExecuteNoneQuery($query);
...
else if($dopost=='delete')
{
    ...
    $rs = $dsql->ExecuteNoneQuery2("DELETE FROM `#@__admin_list` WHERE id='$id' AND id<>1 AND id<>'".$cuserLogin->getUserID()."' ");
    if($rs>0)
    {
        //更新前台用户信息
        $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` SET matt='0' WHERE mid='$id' LIMIT 1");
...
$safecode = substr(md5($cfg_cookie_encode.$randcode),0,24);
$typeOptions = '';
$row = $dsql->GetOne("SELECT * FROM `#@__admin_list` WHERE id='$id'");
...
include XAKINClude('templets/sys_admin_user_edit.htm');