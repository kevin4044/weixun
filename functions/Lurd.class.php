...
class Lurd extends DataListCP
{
    ...
    function MakeListTemplate($listfield = '')
    {
        ...
        {
                $fielditem = "    <td class='nowrap'><input type=\"checkbox\" name=\"{$this->primaryKey}[]\" value=\"{xak:field name='{$this->primaryKey}' /}\" /></td>\r\n";
        }
        else
        {
        ...
        if(!empty($listfield) && $listfield != '*' )
        {
            ...
            foreach($listfields as $k)
            {
                ...
                else
                {
                    ...
                    $fielditem .= "    <td class='nowrap'>{xak:field name='{$k}' $dofunc /}</td>\r\n";
                }
            }//End foreach
        }
        //自动处理
		...
		if($this->singleManage)
		{
			...
			<a href=\"{$currentUrl}?{$this->primaryKey}={xak:field name='{$this->primaryKey}' /}&ac=edit&get=yes\">修改</a> 
			|  <a href=\"{$currentUrl}?{$this->primaryKey}={xak:field name='{$this->primaryKey}' /}&ac=del&get=yes\">删除</a> </td>\r\n";
		}
    ...
    function MakeAddEditTemplate($getTemplets='add')
    {
        ...
        if( !preg_match("/,/", $this->primaryKey) )
        {
            $tempItems['primarykey'] = "    <input type=\"hidden\" name=\"{$this->primaryKey}\" value=\"{xak:field name='{$this->primaryKey}' /}\" />\r\n";
        }
        else
        {
            $prikeys = explode(',', $this->primaryKey);
        ...
        foreach($this->fields as $k=>$v)
        {
            ...
            if( in_array($this->fields[$k]['type'], $this->floatTypes) ) 
            {
                if($getTemplets=='edit')
                {
                    $dfvalue = "{xak:field name='$k' function=\"Lurd::FormatFloat(@me, '$fformat')\" /}";
                }
            ...
            else if( in_array($this->fields[$k]['type'], $this->dateTypes))
            {
                ...
                if($getTemplets=='edit')
                {
                    $dfvalue = "{xak:field name='$k' function=\"MyDate(@me, '$fformat')\" /}";
                }
                else if(empty($this->fields[$k]['default']))
                {
                    $dfvalue = "{xak:var.a function=\"MyDate(time(), '$fformat')\" /}";
                }
            ...
            else if( $this->fields[$k]['type']=='SET' )
            {
                ...
                foreach($ems as $em)
                {
                    ...
                    {
                        $aeform .= "<input type='checkbox' name='{$k}[]' value='$em' {xak:if in_array('$em', \$enumItems)}checked{/xak:if} />$em \r\n";
                    }
                }
            }
            //ENUM类型
            else if( $this->fields[$k]['type']=='ENUM' )
            {
                ...
                {
                    if($getTemplets=='edit') {
                        $aeform .= "<input type='radio' name='$k' value='$em' {xak:if \$fields['$k']}=='$em'}checked{/xak:if} />$em \r\n";
                    }
                    else {
