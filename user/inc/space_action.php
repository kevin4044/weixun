<?php   if(!defined('XAKMEMBER')) exit('Warning...');
/**
 * 空间操作
 * 
...
if($action=='article')
{
    ...
    include_once(XAKINC.'/arc.memberlistview.class.php');
    include_once(XAKINC.'/channelunit.func.php');
    ...
    $dlist->SetTemplate(XAKMEMBER."/space/{$_vars['spacestyle']}/listarticle.htm");
...
else if($action=='viewarchives' && !empty($aid) && is_numeric($aid))
{
    ...
    //解析模板
    $dlist = new MemberListview();
    $dlist->SetTemplate(XAKMEMBER."/space/{$_vars['spacestyle']}/blog.htm");
    $dlist->Display();
    exit();
}
/*---------------------------------
...
else if($action=='archives')
{
    ...
    $dlist->SetParameter("action", $action);
    $dlist->SetTemplate(XAKMEMBER."/space/{$_vars['spacestyle']}/listarchives.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
}
...
else if($action=='album')
{
    ...
    $dlist->SetParameter("uid",$_vars['userid']);
    $dlist->SetParameter("action",$action);
    $dlist->SetTemplate(XAKMEMBER."/space/{$_vars['spacestyle']}/listalbum.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
}
...
else if($action=='guestbook')
{
    ...
    include_once(XAKINC.'/datalistcp.class.php');
    $query = "SELECT mg.*,mb.face,mb.userid,mb.sex From `#@__member_guestbook` mg 
    left join `#@__member_list` mb on mb.userid=mg.gid 
    where mg.mid='{$_vars['mid']}' order by mg.aid desc";
    $dlist = new DataListCP();
    $dlist->pageSize = 10;
    $dlist->SetParameter("uid",$_vars['userid']);
    $dlist->SetParameter("action",$action);
    $dlist->SetTemplate(XAKMEMBER."/space/{$_vars['spacestyle']}/guestbook.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
}
...
else if($action=='friend')
{
    ...
    $dlist->SetParameter("uid",$_vars['userid']);
    $dlist->SetParameter("action",$action);
    $dlist->SetTemplate(XAKMEMBER."/space/{$_vars['spacestyle']}/friend.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
}
...
{
    include_once(XAKDATA.'/enums/nativeplace.php');
    include_once(XAKINC."/enums.func.php");
    $row = $dsql->GetOne("SELECT  * from `#@__member_person` where mid='{$_vars['mid']}' ");
    $dpl = new XakTemplate();
    $dpl->LoadTemplate(XAKMEMBER."/space/{$_vars['spacestyle']}/infos.htm");
    $dpl->display();
}
...
else if($action=='delfriend')
{
    ...
    else
    {
        ...
        #/aip}}
        $inquery = "DELETE FROM `#@__member_friends` where fid='{$_vars['mid']}' And mid='{$cfg_ml->M_ID}' ";
        $dsql->ExecuteNoneQuery($inquery);
        //统计我的好友数量
        $row = $dsql->GetOne("SELECT COUNT(*) AS nums FROM `#@__member_friends` WHERE `mid`='".$cfg_ml->M_ID."'");
        $dsql->ExecuteNoneQuery("UPDATE `#@__member_tj` SET friend='$row[nums]' WHERE `mid`='".$cfg_ml->M_ID."'");
...
{
    $dpl = new XakTemplate();
    $dpl->LoadTemplate(XAKMEMBER."/space/{$_vars['spacestyle']}/introduce.htm");
    $dpl->display();
}
//联系我们
...
{
    $dpl = new XakTemplate();
    $dpl->LoadTemplate(XAKMEMBER."/space/{$_vars['spacestyle']}/contact.htm");
    $dpl->display();
}
/*-------------------------------
function products() { }
...
elseif($action == 'products')
{
    ...
    $dlist->SetParameter('uid', $_vars['userid']);
    $dlist->SetParameter('action', $action);
    $dlist->SetTemplate(XAKMEMBER."/space/{$_vars['spacestyle']}/listproducts.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
}