...
$row=$dsql->GetOne("SELECT  * FROM `#@__member_list` WHERE mid='".$cfg_ml->M_ID."'");
$face = $row['face'];
if($dopost=='save')
{
    $svali = GetCkVdValue();
    ...
    if(!is_array($row) || $row['pwd'] != PassJoin($oldpwd))
    {
    ...
    $query1 = "UPDATE `#@__member_list` SET pwd='$pwd',sex='$sex'{$addupquery} where mid='".$cfg_ml->M_ID."' ";
    $dsql->ExecuteNoneQuery($query1);

    //如果是管理员，修改其后台密码
    if($cfg_ml->fields['matt']==10 && $pwd2!="")
    {
        $query2 = "UPDATE `#@__admin_list` SET pwd='$pwd2' where id='".$cfg_ml->M_ID."' ";
...
include(XAKMEMBER."/templets/edit_baseinfo.htm");