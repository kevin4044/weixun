...
require_once(XAKINC."/arc.caicai.class.php");
...
$query = "Select arc.*,m.userid,m.face,
          ...
          From `#@__archives` arc left join `#@__arctype` tp on tp.id=arc.typeid left join `#@__member_list`  m on m.mid=arc.mid
          where $typequery arc.arcrank>-1
          order by arc.`{$sort}` desc limit $maxrc ";
$dlist->SetParameter('tid',$tid);
$dlist->SetParameter('sort',$sort);
$dlist->SetTemplate(XAKMEMBER.'/templets/caicai.htm');
