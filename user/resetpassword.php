...
require_once(XAKMEMBER."/inc/inc_pwd_functions.php");
...
else if($dopost == "safequestion")
{
    ...
    $sql = "SELECT safequestion,safeanswer,userid,email FROM `#@__member_list` WHERE mid = '$mid'";
    $row = $db->GetOne($sql);
    ...
    elseif($setp == 2)
    {
        ...
        if($row['pwd'] == $sn)
        {
            if($pwd != "")
            {
                if($pwd == $pwdok)
                {
                    $pwdok = PassJoin($pwdok);
                    $sql = "DELETE FROM `#@__pwd_tmp` WHERE `mid` = '$id';";
                    $db->executenonequery($sql);
                    $sql = "UPDATE `#@__member_list` SET `pwd` = '$pwdok' WHERE `mid` = '$id';";
