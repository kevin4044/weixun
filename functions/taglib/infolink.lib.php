...
require_once(XAKDATA.'/enums/nativeplace.php');
require_once(XAKDATA.'/enums/infotype.php');
...
function lib_infolink(&$ctag,&$refObj)
{
    ...
    $fields['linkallplace'] = "<a href='{$baseurl}module/list.php?channelid={$channelid}&tid={$typeid}&infotype={$infotype}'>不限</a>";
    $fields['linkalltype'] = "<a href='{$baseurl}module/list.php?channelid={$channelid}&tid={$typeid}&nativeplace={$nativeplace}'>不限</a>";
    ...
    if(empty($nativeplace))
    {
        foreach($em_nativeplaces as $eid=>$em)
        {
            ...
            $fields['nativeplace'] .= " <a href='{$baseurl}module/list.php?channelid={$channelid}&tid={$typeid}&nativeplace={$eid}&infotype={$infotype}'>{$em}</a>\r\n";
        }
    }
    else
    {
        ...
        $fields['nativeplace'] = "<a href='{$baseurl}module/list.php?channelid={$channelid}&tid={$typeid}&nativeplace={$toptype}&infotype={$infotype}'> <b>{$em_nativeplaces[$toptype]}</b></a> &gt;&gt; ";
        foreach($em_nativeplaces as $eid=>$em)
        {
    ...
    if(empty($infotype) || is_array($smalltypes))
    {
        ...
        foreach($em_infotypes as $eid=>$em)
        {
            ...
            if($eid == $infotype) 
            {
                ...
                $fields['infotype'] .= " <a href='{$baseurl}module/list.php?channelid={$channelid}&tid={$typeid}&infotype={$eid}&nativeplace={$nativeplace}'>{$em}</a>\r\n";
            }
        }
    }
    else
    {
        ...
        $fields['infotype'] .= "<a href='{$baseurl}module/list.php?channelid={$channelid}&tid={$typeid}&infotype={$toptype}&nativeplace={$nativeplace}'><b>{$em_infotypes[$toptype]}</b></a> &gt;&gt; ";
        foreach($em_infotypes as $eid=>$em)
        {
            if($eid < $toptype+1 || $eid > $toptype+499) continue;
            if($eid == $infotype) {
