...
require_once(XAKMEMBER."/inc/inc_catalog_options.php");
require_once(XAKMEMBER."/inc/inc_archives_functions.php");
...
if(empty($dopost))
{
    ...
    include(XAKMEMBER."/templets/album_add.htm");
    exit();
}
...
{
	include(XAKMEMBER.'/inc/archives_check.php');
    $svali = GetCkVdValue();
    ...
    include(XAKMEMBER.'/inc/archives_check.php');
    $imgurls = "{xak:pagestyle maxwidth='$maxwidth' pagepicnum='$pagepicnum' ddmaxwidth='$ddmaxwidth' row='$prow' col='$pcol' value='$pagestyle'/}\r\n";
    $hasone = false;
    $ddisfirst=1;
    ...
    if(!empty($xak_addonfields))
    {
        ...
        if (empty($xak_fieldshash) || $xak_fieldshash != md5($xak_addonfields.$cfg_cookie_encode))
        {
...
'$color','$writer','$source','$litpic','$pubdate','$senddate','$mid','$description','$keywords','$mtypesid'); ";
    ...
    //增加积分
    $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` SET scores=scores+{$cfg_sendarc_scores} WHERE mid='".$cfg_ml->M_ID."' ; ");
    //更新统计
    ...
    if(defined('UC_API') && @include_once XAKROOT.'/api/uc.func.php')
    {
        ...
        $row = $dsql->GetOne("SELECT `scores`,`userid` FROM `#@__member_list` WHERE `mid`='".$cfg_ml->M_ID."' AND `matt`<>10");
