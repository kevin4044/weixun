...
if($step == 1)
{
    ...
    if($dopost=='regbase')
    {
        ...
        {
            $row = $dsql->GetOne("SELECT * FROM `#@__member_list` WHERE uname LIKE '$uname' ");
            if(is_array($row))
            {
        ...
        {
            $row = $dsql->GetOne("SELECT mid FROM `#@__member_list` WHERE email LIKE '$email' ");
            if(is_array($row))
            {
                ShowMsg('你使用的Email已经被另一帐号注册，请使其它帐号！', '-1');
                exit();
        ...
        //检测用户名是否存在
        $row = $dsql->GetOne("SELECT mid FROM `#@__member_list` WHERE userid LIKE '$userid' ");
        if(is_array($row))
        {
            ShowMsg("你指定的用户名 {$userid} 已存在，请使用别的用户名！", "-1");
            exit();
        ...
        $pwd = PassJoin($userpwd);
        
        $spaceSta = ($cfg_mb_spacesta < 0 ? $cfg_mb_spacesta : 0);
        
        $inQuery = "INSERT INTO `#@__member_list` (`mtype` ,`userid` ,`pwd` ,`uname` ,`sex` ,`rank` ,`money` ,`email` ,`scores` ,
       ...
       '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip'); ";
        if($dsql->ExecuteNoneQuery($inQuery))
        {
            ...
            //写入其它默认数据
            $dsql->ExecuteNoneQuery("INSERT INTO `#@__member_flink`(mid,title,url) VALUES('$mid','内容管理系统','http://www.xak.cn'); ");
            ...
            if($cfg_mb_reginfo == 'Y' && $spaceSta >=0)
            {
                ...
                require_once(XAKMEMBER."/templets/reg-new3.htm");
                exit;
            } 
        } else {
    ...
    require_once(XAKMEMBER."/templets/reg-new.htm");
} else {
    if(!$cfg_ml->IsLogin())
    {
    ...
    if($dopost == 'reginfo')
    {
        ...
        {
            $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` SET `spacesta`='2' WHERE `mid`='{$cfg_ml->M_ID}'");
            // 清除缓存
    ...
    require_once(XAKMEMBER."/templets/reg-new2.htm");
}